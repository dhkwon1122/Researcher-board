...
# combined_box/history_box의 폭 계산(calc)에서도 그대로 재사용해야 좌우
# 정렬이 어긋나지 않는다 — 아래 combined_box/history_box 조립부의 calc()
# 식이 전부 이 상수를 참조한다. 사진 자체의 최대 높이(img_max_height)도
...
def _print_profile_content(rid, researcher, tables, profile, name_map,
    ...
    맞춰 테두리 박스로 재배치한다. combined_box는 사진(photo_box)·기본정보+학력
    (info_col)·핵심기술/보유기술(tech_box) 3개를 테두리 하나로 묶은 박스다
    (사용자 요청 — photo_box/tech_box 자체 테두리는 없앰). history_box(논문·특허
    +양성·시상)는 combined_box 바로 아래 별도 박스로, 폭은 combined_box의
    사진+정보 영역과 같게 맞춘다(자세한 폭 계산은 combined_box/history_box
    조립부 주석 참고). 인물 코멘트는 인쇄본에 넣지 않는다(사용자 요청 — 화면
    탭에는 그대로 남아 있음). print_eval_content는 update_profile()이 권한
    ...
    # 사진 박스 — 자체 테두리는 없다(사용자 요청, 아래 combined_box가 사진·
    # 기본정보·핵심기술/보유기술을 한 테두리로 묶는다). 이름/직급연차는
    # photo_block()이 이미 캡션으로 넣어주고, 그 아래에 평가·인센티브
    # 이력을 붙인다(사용자 요청: "평가 인센티브 이력은 사진과 이름 있는
    # 박스 아래에"). display:flex를 인라인 style로 직접 준다(.d-flex
    # 유틸리티 클래스 대신) — 외부 CDN(부트스트랩)이 느리거나 막혀 있어도
    # 항상 세로 배치되게 한다.
    photo_box = html.Div([
        ...
        # 이유). html.Div로 한 번 더 감싸 평평하게 만든다. img_max_height=184
        # (200 → 240(20% 증가) → 204(15% 축소) → 184(사진 열이 combined_box
        # 안에서 제일 길게 내려온다는 피드백으로 10% 추가 축소, 204*0.9=
        # 183.6→184))로 화면(라이브) 탭보다는 여전히 조금 크게 보여준다 —
        # 화면 쪽 호출부(아래 update_profile())는 이 인자를 넘기지 않아
        # 기존 크기(200) 그대로다.
        html.Div(photo_block(rid, name, researcher, CURRENT_YEAR,
                              hide_normal_employment_status=True, img_max_height=184)),
    ...
    # 옮겼다(사용자 요청). 자체 테두리는 없다(사용자 요청, 아래 combined_box
    # 참고). compact=True — 핵심기술은 표 대신 "(등급)분야 > 기술명" 텍스트
    # 나열로, 보유기술은 표 헤더 없이 데이터 행만(둘 다 사용자 요청: "표에는
    # Header가 필요 없을 것 같아").
    tech_box = html.Div([
    ...
    # alignItems는 'stretch'(기본값 명시) — 'flex-start'였으면 각 열이 자기
    # 내용 높이만큼만 차지해 구분선(borderLeft)이 내용이 짧은 열에서는
    # 중간에 끊겨 보였다(사용자 요청: "박스 최상단에서 최하단까지 다
    # 그어지게"). stretch로 바꾸면 세 열 모두 가장 긴 열 높이까지 늘어나
    # (빈 여백은 생기지만 시각적으로 테두리/구분선 안쪽이라 문제 없음)
    # 구분선이 박스 전체 높이를 관통한다.
