...
def _evaluation_period_hint(period: tuple[str, str]) -> str:
    ...
    half_cols = ', '.join(
        f'"{first_half_column(y)}"/"{second_half_column(y)}" (FY{y})'
    ...
    return (
        ...
        f"grade columns (first_half_grade/second_half_grade) are: {half_cols}. "
        "Pick the exact column name from this list that matches the year the "
        "question asks about — do not guess a column name yourself."
    )
