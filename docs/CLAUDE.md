...
## 2026-09-11 (3): build_team_refer_intake.py — 1단계부서명이 "대표이사"면
2단계부서명도 동일하게 반영

사용자 요청: 1단계부서명 값이 "대표이사"인 행은 같은 행의 2단계부서명도
"대표이사"로 맞춰달라는 것 — 기존 `_fill_upper_level()`의 g-1 규칙(1단계가
"대표이사"/"삼성전자"면 1단계를 2단계 값으로 교체)과는 반대 방향으로,
"대표이사"는 그 자체로 이미 최상위 단위라 원본 2단계 값(현소속부서명)이
뭐든 무시하고 1/2단계를 "대표이사"로 통일하자는 것.

신규 `_MIRROR_TO_LEVEL2 = {'대표이사'}` — 원본 행 추출 직후, `_fill_upper_level()`
호출보다 먼저 `if a in _MIRROR_TO_LEVEL2: b = a`로 2단계값을 1단계와
동일하게 맞춘다(순서가 중요 — 먼저 b:=a로 통일해두면 이후 g-1 규칙
"a=b로 교체"가 실행돼도 이미 a=b라 값이 그대로 유지됨). "삼성전자"는 이
대상에서 빠져 있어 기존 g-1 규칙(1단계를 2단계 값으로 교체) 그대로 유지
— 사용자가 "대표이사"만 지목했으므로 범위를 좁혔다.

검증: (1) 1단계="대표이사"+2단계="비서실"(다른 값) → 1/2단계 모두
"대표이사"로 통일되는 것, (2) 1단계="대표이사"+2단계 공백 → 마찬가지로
"대표이사"로 채워지는 것, (3) 1단계="삼성전자"(대상 아님) → 기존 규칙
그대로 2단계 값("인사팀")으로 1단계가 교체되는 회귀 없음, (4) 기존
백필 회귀 테스트(종합기술원 root marker, 보조 lookup 백필)가 전과 동일한
결과를 내는 것 — 4가지 모두 합성 데이터로 확인. `python3 -c "import app"`
전체 임포트 확인, `py_compile` 통과.
