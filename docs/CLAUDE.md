...
- 평가등급 권한이 없는 사용자(talent_dev)로 별도 계정을 만들어 확인 —
  ...
  운영 환경에서는 정상 렌더링됨 — docs/CLAUDE.md의 기존
  기록 참고).
...
## 2026-09-10 (4): 로그인/초기설정 화면에 남아있던 "SAIT in 360°" 누락분 수정

사용자가 실제로 로그인 화면을 확인해보니 "People in SAIT"가 아니라 여전히
"SAIT in 360°"로 보인다고 리포트 — 앞선 (3)번 작업에서 저장소 전체를
"SAIT in 360" 문자열로 재검색해 확인했었는데, **로그인/초기설정 화면
(`_html_page()`)의 문구는 `<span>` 태그로 "S"/"AI"/"T in 360°"가 쪼개져
있어(`S<span style="color:#1677ff">AI</span>T in 360&deg;`) 그 문자열
검색에 걸리지 않았다** — 순수 텍스트 그레핑의 한계로 놓친 케이스.

**`app.py`**: `_html_page()`(로그인/초기설정 공통 HTML 템플릿)의
`<h5>` 문구를 `S<span>AI</span>T in 360&deg;` → `People in
S<span>AI</span>T`로 수정(파란색 강조 span은 그대로 유지). 이 템플릿은
`_brand_label()`을 재사용하는 게 아니라 완전히 별도로 하드코딩된 HTML
문자열이라, (3)번에서 `_brand_label()`만 고치고 이쪽은 놓쳤던 것 —
`_brand_label()` docstring의 "로그인/초기설정 화면도 같은 문구를
재사용한다"는 설명은 실제로는 사실이 아니었다(개념적으로만 같은 문구를
의도했을 뿐, 코드 상 두 곳이 독립적으로 하드코딩돼 있었음).

검증: `_html_page()`를 직접 호출해 렌더링된 HTML에서 `<h5>` 태그 내용이
`People in S<span style="color:#1677ff">AI</span>T`로 나오는 것 확인.
저장소 전체를 `"in 360"`/`"S<span"`/`"AI</span>T"` 패턴으로 재검색해
(`services/dev_updates.py`의 날짜별 변경 이력 문구 제외) 더 이상 남은
곳이 없는 것 확인. `py_compile` + `import app`으로 전체 임포트 확인.

**교훈**: 이번처럼 브랜드 문구가 HTML/JSX 마크업으로 쪼개져 있으면
(`f"S<span>...</span>T in 360"`처럼) 순수 문자열 그레핑("SAIT in
...
## 2026-09-15: 팀/리더 참조 그리드 — 체크박스 선택 이동/삭제 +
"(SAIT)"/"(기술원)" 표기 자동 제거

사용자 요청 두 가지: (1) 팀/리더 참조 탭에서 행별 체크 또는 전체 체크 후
"삭제", "위로/아래로" 이동이 가능하게 해달라. (2) 1~3단계부서명에
"(SAIT)" 또는 "(기술원)"이라는 텍스트가 있으면 지우고 보여달라(예:
"AI융합기술팀(SAIT)"/"AI융합기술팀(기술원)" → "AI융합기술팀"). 세부
설계는 여러 차례 예시 기반 확인을 거쳐 다음과 같이 최종 확정됐다:
이동/삭제는 하위 조직(자식 행)을 자동으로 함께 포함하되(부모 행 체크 시
그 아래 전체 서브트리가 같이 움직임/삭제됨), 이동은 **같은 상위부서
형제 사이에서만** 허용하고(부모-자식 간 이동/재배치는 불가) 다른
상위부서 경계를 넘으려 하면 "다른 상위부서로의 이동은 불가합니다"
문구를 보여준다. 이동은 어디까지나 형제 간 표시 순서(dep_code)만
바꾸는 것이고 1/2/3단계부서명 자체를 고쳐 실제 조직을 재배치하는 것은
아니다. 부모-자식 관계는 자동 하이라이트가 아니라 "구조" 열의
아이콘/들여쓰기로 사전에 항상 보이게 한다. 태그 제거는 화면 표시만이
아니라 원본 데이터(team_refer.csv) 자체에서 지워서 저장하되, 서로 다른
원래 표기가 태그 제거로 우연히 하나의 조직으로 합쳐지는 경우를 대비해
저장 시 병합 확인 경고를 추가로 보여준다.

**`pipeline/team_hierarchy.py`**: 다른 모듈(`process_team_refer.py`)이
재사용할 수 있도록 `_LEVEL_FIELDS`→`LEVEL_FIELDS`, `_slug`→`slug`,
`_own_path`→`own_path`로 공개 이름으로 변경(내부 동작 변경 없음).
`derive_hierarchy()` 안에서 `own_path(record)` 결과를 담던 지역 변수
이름이 우연히 함수명과 같아 `UnboundLocalError`를 일으켰던 것을
`record_path`로 고쳐서 해결.

**`pipeline/process_team_refer.py`**: `_ORG_TAG_PATTERN =
re.compile(r'\(SAIT\)|\(기술원\)')`로 두 태그를 찾아 지우는
`_strip_org_tags()`/`_strip_record_tags()` 신설. 기존 `build_rows_from_
records()`가 레코드를 정제(컬럼 매핑 등)하는 로직을 `_cleaned_records()`
로 뽑아내 공통화한 뒤, 태그 제거 → `derive_hierarchy()` 순서로
파이프라인을 구성(정제 로직이 두 곳에서 갈라지지 않도록 함). 신규
`find_tag_merges(records)`: 같은 레코드 집합을 "태그 제거 전/후" 두
번 처리해서 태그 제거 후 own_path가 같아진 그룹 중 태그 제거 전
own_path가 서로 달랐던 경우만 "병합"으로 분류하고, 실제로 저장될 최종
값(`derive_hierarchy()`가 필드별로 "먼저 채워진 값 우선"으로 결정한
값)과 병합 전 각 원본의 값을 나란히 반환한다. `process()`(엑셀 일괄
업로드 경로)에도 이 병합 목록을 콘솔에 경고로 출력하는
`_print_merge_warning()`을 연결.

**`services/team_refer_store.py`**: `save_snapshot()`이
`find_tag_merges(records)`를 호출해 반환값에 `tag_merges` 키로 담아서
돌려주도록 확장(기존 `duplicate_dep_ids`와 같은 패턴).

**`pages/admin.py`** — 그리드에 `row_selectable='multi'`(왼쪽 체크박스
열)를 추가하고 `_no`/`_tree`(구조, 부모-자식 아이콘) 두 개의
비편집 열을 신설. `_tree` 열은 `_annotate_tree()`가 행 배열을 한 번
스캔하며(스택 기반) "보이는 조상만 기준으로 한" 들여쓰기 깊이를
계산해서 최상위는 🏢, 자식이 있는 팀은 🗂️ (+N), 자식 없는 팀은 🗂️,
3단계(리프)는 └ 📄 아이콘으로 표시한다(숨김 처리된 행은 배열 자체에
없으므로 자동으로 건너뛰어져, 숨은 조상 밑의 행이 엉뚱하게 들여써지지
않음). 서브트리/형제 판정 알고리즘(JS 드래그 기능과 별개로 파이썬
쪽에도 새로 포팅 — 서로 다른 데이터 표현이라 코드 공유 불가): `_row_
path`(1~3단계 열을 앞에서부터 읽어 처음 나오는 빈 값 직전까지),
`_is_descendant_path`(엄격한 접두사 판정), `_subtree_range`(연속된
자손 블록 범위), `_preceding_sibling_range`/`_following_sibling_start`
(깊이와 부모 경로가 정확히 같은 인접 형제 블록을 앞/뒤로 찾고, 부모가
다르거나 더 얕은 행을 만나거나 배열 끝에 닿으면 `None`을 반환해 "이동
불가" 경계를 정확히 판정), `_reassign_sibling_codes`(형제들의 dep_code를
새 순서에 맞게 재부여).

신규 콜백 3개: `team_refer_bulk_delete`(체크된 행 + 그 서브트리를 모두
삭제), `team_refer_move_selected`(위로/아래로 버튼 — `ctx.triggered_id`
로 두 버튼을 한 콜백에서 구분, 체크된 각 행을 형제 범위 안에서 한
칸씩 개별 이동시키고 경계에 걸리면 카운트만 하고 스킵), `team_refer_
select_all`/`team_refer_select_none`(아래 참고). 두 콜백 모두 헤더
정렬(`sort_by`) 중에는 계층 판정이 부정확해지므로 기존 드래그 기능의
`window.__teamReferSortActive` 가드와 동일한 취지로 이동/삭제를 막고
안내를 띄운다. 기존 `team_refer_add_row`/`team_refer_sort`/`data_
update_poll` 콜백에는 각각 `selected_rows`를 `[]`로 리셋하는 Output을
추가해, 행 추가/정렬/자동 갱신 후 체크 상태가 엉뚱한 행을 가리키게
남아있지 않도록 함.

**dash_table 자체 한계 발견**: `row_selectable='multi'`를 켜도
dash_table은 헤더에 "전체 선택" 체크박스를 자체적으로 렌더링하지
않는다(`dash-select-header` 셀이 항상 빈 채로 남음 — 라이브러리
자체의 알려진 제약이지 이 앱의 버그가 아님을 실제 렌더링된 DOM을
직접 열어서 확인). 그래서 헤더 체크박스 대신 "전체 선택"/"전체 해제"
버튼 두 개를 툴바에 추가해 동일한 기능을 대체 제공(`team_refer_select_
all`은 `selected_rows`를 `range(len(rows))`로, `team_refer_select_none`
은 `[]`로 설정).

**실제로 발견해 고친 회귀 버그(사용자가 보고한 게 아니라 라이브 테스트
중 콘솔 에러 로그를 보고 직접 발견)**: `assets/team_refer_grid.js`의
기존 `mouseup` 핸들러(클릭한 위치로 텍스트 커서를 옮겨주는 기능,
2026-09-03 도입)가 `input.tagName === 'INPUT'`만 검사하고 `type`은
안 봐서, 이번에 추가된 체크박스 `<input type="checkbox">`에도 반응해
`setSelectionRange()`를 호출하다가 `DOMException`을 던지고 있었다.
`input.type`이 `'text'`가 아니면 그냥 리턴하도록 가드를 추가해 해결.

**병합 확인 모달(`team-refer-merge-modal`)**: 기존 중복 dep_id 경고
모달과 같은 패턴으로 신설. `_merge_modal_body()`가 병합 그룹마다 "■
OOO으로 합쳐짐" 구분 행 아래에 각 원본 표기와 조직코드/사번/성명/직책을
연한 회색으로, 최종 유지된 값을 초록색 행으로 나란히 보여줘서 "값이
섞여서 유실된 게 없는지" 한눈에 대조할 수 있게 했다. `team_refer_save()`
가 `result['tag_merges']`를 받아 있으면 저장 메시지에 안내를 덧붙이고
모달을 연다. **라이브 검증 중 발견해 고친 버그**: 최초 구현에서
"최종 유지" 행이 "조직코드" 열에 `org_name_wd`(비공식소속부서명에
대응하는 필드)를 표시하고 있었는데, 바로 위 "원본" 행들은 같은 열에
`dep_code`(그리드의 "조직코드" 열이 실제로 매핑되는 필드,
`_COL_MAP`상 "조직코드"→`dep_code`)를 표시하고 있어 열 이름과 값의
의미가 행마다 어긋나 있었다. "최종 유지" 행도 `dep_code`를 쓰도록
고쳐서, 같은 열 안에서 원본↔최종 값이 실제로 비교 가능하도록 맞췄다.

**검증**: 실제 서버(임시 admin 계정)에서 Playwright로 (1) 자식이 있는
행(체크 시 자식도 함께 선택되는 게 아니라, 체크한 행의 서브트리
범위를 서버 쪽에서 계산해 처리하는 방식이므로 부모 행 하나만 체크해도
그 서브트리 전체가 이동/삭제됨)을 위로 이동 → 순서와 dep_code 재부여가
기대한 그대로인 것, (2) 형제가 없는 경계에서 이동 시도 → "다른
상위부서로의 이동은 불가합니다" 정확히 노출, (3) 체크 삭제 시 서브트리
전체 삭제 및 안내 문구, (4) 전체 선택/전체 해제 버튼이 실제 그리드
데이터 행 수만큼만(헤더 행 제외) 정확히 체크/해제되는 것, (5) 콘솔/
페이지 에러 0건(체크박스 회귀 버그 수정 후 재확인), (6) "(SAIT)"/
"(기술원)" 태그가 붙은 새 행을 그리드에서 직접 입력해 저장 → 기존
동일 조직과 합쳐져 총 행수가 늘지 않고, 병합 확인 모달에 원본 2건과
최종 유지 값이 조직코드 열 기준으로 일관되게 표시되는 것까지 확인.
`python3 -m py_compile`/`python3 -c "import app"` 통과. 테스트 계정·
`data/processed/team_refer.csv`(및 `team_leader_refer/` 산출물)는 검증
후 삭제, `config/users.json`은 원본과 diff 없음 재확인.

**미검증**: 매우 깊은 트리(4단계 이상)나 형제가 수십 개인 대규모
실데이터에서 이동 버튼 클릭 시 체감 반응 속도, 여러 행을 동시에 체크한
채 반복 이동할 때(형제 경계에 막힌 행이 선택 상태를 유지한 채로 다음
조작에 영향을 주는지) 실사용 흐름에서 혼란을 주지 않는지.

## 2026-09-16: 팀/리더 참조 그리드 — "구조" 열 제거·이동 단순화·조직코드
전역 재배번 + 신규 업로드 시 책임자 정보 과거값 자동 채움

바로 전날(2026-09-15) 만든 체크박스 이동/삭제 기능을 사용자가 실사용
관점에서 다시 검토하고 4가지 수정을 요청했다: (1) "구조" 열(부모-자식
아이콘 표시) 제거 — 관련 데이터도 함께 삭제. (2) 이동을 "같은 상위부서
형제 안에서만"이 아니라 상위부서 경계와 무관하게 자유롭게 가능하도록
(단, dep_id/upper_dep_id 등 조직도 생성에 쓰이는 실제 부모-자식 관계
데이터는 그대로 유지). (3) 행 이동에 따라 조직코드가 자동으로 정리되게
(맨 위가 1, 맨 뒤가 N). (4) 신규 데이터 업로드 시 사번/성명/직책을 과거
같은 비공식소속부서명 기준으로 자동 채우고, 관리자가 수정하면 이후
업로드부터는 그 수정된 값이 자동 채워지도록.

**(2)의 설계 확인**: "구조" 열을 없애면서 기존의 "체크한 부모 행을
이동/삭제하면 하위 조직 전체가 함께 움직인다"는 동작을 어떻게 할지가
불명확해(부모-자식 관계를 미리 보여주던 화면 장치 자체가 없어지므로)
사용자에게 직접 확인했다: **이동은 체크한 행 하나만 개별로** 움직이고
(하위 조직을 묶어서 옮기지 않음), **삭제는 기존처럼 하위 조직 전체를
함께** 지운다(하위 조직 행이 남아있으면 그 소속 정보 때문에 상위 조직이
`derive_hierarchy()`에 의해 자동으로 다시 만들어져 사실상 삭제되지
않는 문제가 있음을 설명하고 확정).

**`pages/admin.py`**:
- `_TREE_ICONS`, `_annotate_tree()` 삭제. `_renumbered()`는 이제 'No.'만
  다시 매기고 '_tree' 관련 로직은 전혀 건드리지 않는다 — 이 함수가
  "직군 예외자" 탭과도 공유되는 범용 헬퍼라는 걸 다시 확인했고(그 탭
  행에는 애초에 '_tree'가 무의미한 필드였다), 제거가 그 탭에도 영향
  없음을 확인.
- `_preceding_sibling_range()`/`_following_sibling_start()`/
  `_reassign_sibling_codes()`(형제 그룹 안에서만 이동시키던 경계 판정 +
  형제 그룹별 조직코드 재배당) 전부 삭제.
- `_move_selected()`를 전면 단순화: 체크한 각 행의 인덱스를 찾아 방향에
  맞는 처리 순서로 정렬한 뒤, 바로 위/아래 행과 자리를 1칸씩 맞바꾼다
  (`rows[idx-1], rows[idx] = rows[idx], rows[idx-1]` 형태) — 하위 조직
  탐색(`_subtree_range`)을 이동에는 더 이상 쓰지 않는다(삭제에는 계속
  사용). 그리드 맨 위/맨 아래에 이미 있어 더 못 움직이는 행만 `blocked`
  로 세고, 안내 문구를 "다른 상위부서로의 이동은 불가합니다"에서 "이미
  맨 위/맨 아래에 있어 더 이상 이동할 수 없는 행이 있습니다"(색상도
  warning→info)로 변경.
- 신규 `_renumber_dep_codes(rows)`: 현재 화면 순서 그대로 조직코드를
  1~N으로 다시 매긴다. `_renumbered()`와 분리한 이유 — `_renumbered()`는
  헤더 클릭 정렬(`team_refer_sort`)과 초기 로드에서도 쓰이는데, 정렬은
  "보여주는 순서만" 바꾸는 기능이라 그 결과로 조직코드까지 덮어써 버리면
  안 되기 때문(정렬 기준으로 조직코드가 영구히 바뀌어 버리는 부작용
  방지). `_renumber_dep_codes()`는 이동(`team_refer_move_selected`)·
  삭제(`team_refer_bulk_delete`)·행 추가(`team_refer_add_row`) 콜백에서만
  명시적으로 호출한다. 조직코드는 조직도(`pipeline/rd_specialist_
  markdown.py`의 `items.sort(key=dep_code)`)가 같은 부모 아래 형제
  노드의 표시 순서로도 그대로 쓰는데, 전역 단조증가 값으로 다시 매겨도
  부분집합(같은 부모의 형제들)만 봤을 때 상대적 순서는 그대로 보존되므로
  조직도 렌더링에는 영향이 없다(관리자 화면에서 상위부서 경계를 자유롭게
  넘나들며 재배열해도 무방한 이유).
- 네이티브 ×(단일 행 삭제, `row_deletable`)는 클라이언트에서 바로
  `data`를 줄이고 별도 콜백을 안 거치는데, 기존에 이미 있던 "No.가
  어긋나면 다시 매긴다" 감시 콜백(`team_refer_renumber_on_change`)이
  바로 그 시점을 잡아내는 유일한 지점이라 여기에도 `_renumber_dep_codes`
  호출을 추가해 × 삭제 후에도 "맨 위 1~맨 뒤 N" 규칙이 깨지지 않게
  했다 — 이 콜백은 이미 idempotent 가드(No.가 이미 맞으면 no_update)가
  있어서, 정렬처럼 이 콜백이 굳이 손댈 필요 없는 변경(정렬 콜백이 이미
  No.를 맞춰서 반환)에서는 이 가드에서 즉시 멈춰 조직코드에 영향이
  없다.
- `columns`에서 `{'name': '구조', 'id': '_tree', ...}` 제거,
  `style_cell_conditional`의 `_tree` 항목 제거, 툴바 안내 문구를 새
  동작(개별 이동/자유 이동/삭제는 하위 포함/이동 후 조직코드 자동
  정리)에 맞게 재작성.

**`pipeline/process_team_refer.py`** — 신규 `_backfill_leader_fields
(result)`: `scripts/build_team_refer_intake.py`가 만드는 인텔이크는
사번/성명/직책이 항상 빈 값인데(그 스크립트 자체가 "이 조직의 책임자가
누구인지"는 판단 범위 밖이라고 명시), 매번 관리자가 다시 채워 넣지
않아도 되도록 같은 비공식소속부서명(org_name_wd — researchers.csv의
org_code와 매칭되는, 조직이 트리에서 위치를 옮겨도 안정적인 키)을 가진
"현재" 조직(`rd_specialist_markdown.read_team_refer()`가 반환하는,
톰스톤되지 않은 가장 최근 저장값)의 값으로 채운다. 이번 업로드에 이미
값이 있는 필드는 덮어쓰지 않는다(비어 있을 때만 채움 — 사람이 직접
채워 올리는 팀참조시트.xlsx 업로드를 그대로 존중). `process()`(xlsx/csv
일괄 업로드)에서 `build_rows_from_records()` 직후, 중복/병합 경고 계산
전에 호출. **`services.team_refer_store.save_snapshot()`(관리자 화면
그리드 저장)에는 의도적으로 적용하지 않는다** — 그리드는 매번 "현재
전체 + 편집분"을 다시 제출하는 구조라, 관리자가 담당자 공석 처리 등
목적으로 일부러 값을 비워 저장해도 여기서 과거값으로 도로 채워버리면
의도적인 공백 처리가 아예 불가능해지기 때문. 이 설계 덕분에 "관리자가
수정하면 이후 업로드부터 그 수정값이 자동 채워진다"(사용자 요청 4번)는
별도 이력 저장소 없이 자연히 성립한다 — team_refer.csv 자체가 항상
"마지막으로 확인된 값"의 원천이므로, 그리드에서 수정 후 저장하면 그
값이 다음 `read_team_refer()` 조회에 바로 반영된다.

**검증**:
- 단위 테스트(`python3 -c` 직접 호출): `_move_selected()`로 자식이 있는
  B2팀을 체크해 위로 이동시켰을 때 자식 B2-1파트가 따라오지 않고 B2팀만
  단독으로 이동하는 것, B3팀을 5번 연속 위로 이동시켜 B2-1파트→B2팀→
  B1팀을 순서대로 넘어 그리드 맨 위(인덱스 0)까지 개별적으로 도달한 뒤
  더 이상 못 올라가고 `blocked=1`이 되는 것(다른 상위부서 소속인
  A연구소 그룹과 무관하게 자유롭게 이동), `_renumber_dep_codes()`가
  1~N 순서를 정확히 매기는 것, `_subtree_range()`(삭제용)는 그대로
  B2팀+B2-1파트를 하나의 구간으로 묶는 것을 각각 확인.
- `_backfill_leader_fields()`: (a) 과거 저장값이 있는 조직에 빈 사번/
  성명/직책으로 재업로드하면 과거값이 채워지는 것, (b) 이미 값이 채워진
  업로드는 덮어쓰지 않는 것, (c) 실사용 흐름 전체 재현 — 최초 업로드
  (빈 값) → 관리자가 그리드에서 수동 수정·저장(`save_snapshot`) →
  이후 새 업로드(다시 빈 값)를 처리하면 **수정된 값**이 채워지는 것
  (원래 값이 아니라)까지 3단계 전체를 확인.
- 실제 서버(임시 admin 계정) + Playwright: (1) 그리드에 "구조" 열이
  전혀 렌더링되지 않는 것, (2) 자식이 있는 B2팀을 체크해 아래로 이동시켜
  실제로 자기 자식(B2-1파트)과 자리를 맞바꾸는(=묶여서 이동하지 않는)
  라이브 동작, (3) 체크한 행을 삭제하면 하위 조직까지 함께 사라지고
  "저장" 후 실제 CSV에도 반영되는 것, (4) 이동/삭제 후 조직코드가 화면
  순서 그대로 재배번되고, "저장" 후 CSV의 dep_id/upper_dep_id는
  재배번과 무관하게 원래 부모-자식 관계 그대로 보존되는 것(예: B2-1파트가
  화면상 B2팀보다 위로 옮겨져도 upper_dep_id는 여전히 B2팀의 dep_id를
  가리킴)을 CSV 직접 열람으로 확인, (5) 콘솔/페이지 에러 0건. 테스트
  계정·`data/processed/team_refer.csv`(및 `team_leader_refer/` 산출물)는
  검증 후 삭제, `config/users.json`은 원본과 diff 없음 재확인.
  `python3 -m py_compile`/`python3 -c "import app"` 통과.

**미검증**: 실제 인력현황 원본 파일(`scripts/build_team_refer_intake.py`
산출물)을 웹 업로드 UI로 직접 올려 `_backfill_leader_fields()`가 실사용
규모(수백 개 조직 단위)에서도 체감 성능·정확도 문제없이 동작하는지,
...
## 2026-09-16: 팀/리더 참조 — 체크박스 선택/이동/삭제 버튼 스크롤해도
항상 보이게(sticky)

사용자 요청: 전체 선택/전체 해제/위로/아래로/선택 삭제 버튼이 그리드
위쪽에 고정돼 있어서, 아래쪽 행을 다루려면 위로 스크롤해서 체크박스나
버튼을 누른 뒤 결과를 보려고 다시 아래로 스크롤해야 하는 왕복이 불편함
— 이 버튼들이 스크롤해도 항상 화면에 보이면 좋겠다는 요청.

**구현**: 이 5개 버튼(+안내 문구)을 기존 3열 툴바 `dbc.Row`에서 분리해
별도 `html.Div(..., className='team-refer-sticky-toolbar')`로 빼고,
`assets/custom.css`에 `position: sticky; top: var(--app-navbar-height,
60px); z-index: 1010;`을 추가했다. 나머지 버튼(행 추가/저장/엑셀
다운로드/DB 반영, 입력 날짜)은 사용자가 지목한 대상이 아니라 기존
위치(스크롤에 따라 함께 스크롤됨)에 그대로 뒀다 — 한 `dbc.Row`의 일부
컬럼만 sticky로 빼면 그 컬럼이 빠진 자리에 빈 공간이 남는 부자연스러운
레이아웃이 되므로, 아예 별도 블록으로 분리하는 편이 깔끔했다.

`top` 값을 고정 픽셀로 하드코딩하지 않고 CSS 변수
(`--app-navbar-height`)로 뺀 이유: 네비게이션 바(`dbc.Navbar(...,
sticky='top')`)가 브랜드/아이콘 행 + 메뉴 링크 행 2단으로 구성돼 있어
실제 렌더링 높이가 폰트 로딩 상태에 따라 달라질 수 있고, 하드코딩된
값이 나중에 네비게이션 바 디자인이 바뀌면 조용히 어긋날 위험이 있다.
대신 `assets/team_refer_grid.js`에 `syncNavbarHeightVar()`를 추가해
`.app-navbar`의 실제 `getBoundingClientRect().height`를 재서 그 CSS
변수를 채워 넣는다.

**실제로 발견해 고친 타이밍 버그**: 처음에는 `window.addEventListener
('load', syncNavbarHeightVar)`만으로 충분할 거라 생각했는데, 실제
브라우저에서 확인해보니 `load` 이벤트가 끝난 뒤에도 `--app-navbar-height`
가 계속 빈 값이었다 — Dash는 React SPA라 브라우저의 `load` 이벤트가
실제 React가 네비게이션 바를 화면에 그리기 전에 먼저 끝나버리는
경우가 있었다(`.app-navbar`가 그 시점에 아직 DOM에 없어 함수가 조용히
아무 일도 안 하고 끝남, 이후 재시도 없음). 이 파일이 이미 그리드 관련
로직에서 쓰던 것과 같은 방식으로, `window.__teamReferOnDataChange`
(pages/admin.py의 clientside_callback이 team-refer-table의 data/sort_by가
바뀔 때마다 부르는, 그리드가 실제로 화면에 존재해야만 호출되는 함수)
안에서도 `syncNavbarHeightVar()`를 다시 부르도록 고쳐서 해결 — 이
시점엔 네비게이션 바도 항상 이미 렌더링돼 있다.

**검증 중 발견한 사실(이 개발 환경 한정 이슈, 앱 버그 아님)**: 실제
브라우저로 스크롤 동작을 확인하는 과정에서, 이 샌드박스가 Bootstrap
CSS를 CDN(jsdelivr)에서 못 받아오는 걸 발견했다(`net::
ERR_TUNNEL_CONNECTION_FAILED` — 이 환경의 네트워크 프록시가 그 도메인을
막음). Bootstrap CSS가 없으면 `dbc.Tabs`가 쓰는 `.tab-pane:not(.active)
{display:none}` 규칙이 적용되지 않아 관리자 화면의 6개 탭 콘텐츠가
전부 한 페이지에 쌓여서(숨겨져야 할 탭까지 전부 보임) 문서 높이가
비정상적으로 커진다 — 그 상태에서 "페이지 맨 아래까지" 스크롤하면
실제로는 "팀/리더 참조" 탭 자신의 콘텐츠 범위를 훨씬 지나 다른 탭들
콘텐츠까지 스크롤한 것이라, sticky 요소가 (정상적인 CSS 동작으로) 자기
컨테이닝 블록 끝을 지나며 화면 밖으로 사라지는 게 마치 "sticky가 아예
안 먹힌다"처럼 보였다. 실제로는 스크롤 거리를 팀/리더 참조 탭 자신의
콘텐츠 범위(이 테스트에서는 25행 기준 약 1250px) 안으로 제한해서
확인하니 `top` 값(158px)에 정확히 도달한 뒤 계속 고정되는 정상적인
sticky 동작이 확인됐다 — 즉 버그가 아니라 이 샌드박스의 CDN 차단이
만든 착시였다. 실사용 환경(운영 서버)은 Bootstrap CSS가 정상적으로
로드돼 비활성 탭이 제대로 숨겨지므로 이 문제와 무관하다.

**검증**: 실제 서버(임시 admin 계정) + 25행 합성 데이터로 Playwright
확인 — (1) `--app-navbar-height` CSS 변수가 실제 네비게이션 바 높이로
정확히 채워지는 것, (2) 스크롤 거리를 늘려가며(200→600px) 툴바의
`top`이 점점 줄어들다 `--app-navbar-height` 값에 도달한 뒤 계속
고정되는 것(정상 sticky 동작), (3) 그 상태(위쪽 툴바가 화면에 보이는
채로 맨 아래쪽 행이 뷰포트 안에 들어온 상태)에서 실제로 맨 아래 행을
체크하고 "위로" 버튼을 눌러 정상 동작하는 것까지 확인. `python3 -m
py_compile`/`python3 -c "import app"` 통과. 콘솔/페이지 에러 0건(CDN
차단으로 인한 리소스 로드 실패 메시지 제외). 테스트 계정·
`data/processed/team_refer.csv`는 검증 후 삭제, `config/users.json`은
원본과 diff 없음 재확인.

**미검증**: 실제 운영 서버(Bootstrap CSS 정상 로드)에서 육안으로도
버튼이 스크롤 중 계속 보이는지(로직/좌표 계산은 확인했으나 이
개발 환경에서는 CDN 차단으로 실제 렌더링 스타일까지 완전히 동일한
조건에서 스크린샷으로 재현하지 못함), 네비게이션 바가 화면 폭에 따라
줄바꿈 형태가 달라지는 경우(예: 매우 좁은 모바일 폭에서 메뉴가 3단
이상으로 늘어나는 경우) `resize` 이벤트로 그 변화까지 정확히 다시
반영되는지.

## 2026-09-17: 팀/리더 참조 — 신규 업로드 책임자 자동 채움에 "미래 시점만"
조건 추가

사용자 질문: 2026-09-16에 넣은 "비공식소속부서명 기준 사번/성명/직책
자동 채움" 기능이 실제로 반영돼 있는지 확인 요청 — 확인 결과 반영돼
있음(`pipeline/process_team_refer.py`의 `_backfill_leader_fields()`).
이어서 로직 수정 요청: 무조건 "가장 최근 저장값"을 채우지 말고, **이번
업로드의 유효 날짜가 그 조직의 기존 저장값 날짜와 같거나 그 이후일
때만** 채우도록. 예시로 확인: AI융합팀이 2026-09-04일자로 이미
사번/성명/직책이 저장돼 있을 때, 2026-09-14일자로 업로드하면(미래
시점) 그대로 채우고, 2026-09-01일자로 업로드하면(9/4보다 과거 시점)
채우지 않고 공란으로 남긴다.

**배경 — 왜 이 조건이 필요한가**: 기존 로직은 valid_date와 무관하게
무조건 "현재(가장 최근) 저장값"을 채웠다. 그런데 team_refer는 과거
시점을 소급 입력하는 것도 지원하는 구조라(process()의 valid_date
인자), 만약 9/4에 새로 알게 된 값(예: 그 시점에 새로 배정된 책임자)을
9/1(9/4 이전 시점)에도 똑같았다고 가정해 채워버리면, 실제로는 9/1
시점의 책임자가 달랐을 수도 있는데 잘못된 값을 소급 적용하는 셈이
된다 — "미래로만 이어 채운다"는 방향이 안전하다.

**`pipeline/process_team_refer.py`**: `_backfill_leader_fields(result,
valid_date)`로 시그니처 변경(기존에는 `result`만 받음). 내부에 신규
`_prev_valid_date(prev)` 헬퍼 — "현재" 조직 레코드의
valid_year/valid_month/valid_day를 `date` 객체로 조립하고(옛 스키마 등
값이 없거나 해석 불가능하면 `None` — 이 경우 안전하게 채우지 않음),
`_fill()`에서 `valid_date >= prev_date`일 때만(같은 날짜도 포함) 채우고
그 외(과거 시점 소급 입력)에는 원래 빈 값 그대로 둔다. `process()`에서
`valid_date = valid_date or date.today()` 계산을 `_backfill_leader_fields()`
호출보다 앞으로 옮겨서 그 값을 넘겨준다(기존에는 `stamp_valid_date()`
직전에만 계산했음).

**검증**: `python3 -c` 직접 호출로 사용자가 든 예시 그대로 재현 —
AI융합팀을 2026-09-04일자·사번 00001234·성명 홍길동·직책 부사장으로
먼저 저장한 뒤, (1) 2026-09-14(9/4보다 미래)로 빈 값 업로드 →
정확히 채워짐, (2) 2026-09-01(9/4보다 과거)로 업로드 → 공란 유지,
(3) 정확히 9/4(경계값, 같은 날짜)로 업로드 → 채워짐(">=" 조건이므로
포함) 세 가지 모두 확인. 실제 `process()` 함수를 통한 종단 간 테스트
(임시 업로드 폴더 + csv 파일)로 9/1(과거) 업로드 시 실제
`team_refer.csv`에 빈 값으로 새 행이 추가되고 기존 9/4 행(홍길동)은
그대로 보존되는 것까지 확인. `python3 -m py_compile`/`python3 -c
"import app"` 통과.

**미검증**: valid_year/month/day가 비정상적인 값(예: 2월 30일처럼
`date()` 생성자가 예외를 던지는 값)으로 이미 저장돼 있는 실제 운영
데이터가 있는지(있다면 `_prev_valid_date()`가 `None`을 반환해 안전하게
채우지 않는 쪽으로 처리되지만, 이런 데이터가 실제로 존재하는지는
확인하지 못함).

## 2026-09-17: 팀/리더 참조 — 드래그 이동/체크박스 다수 선택 시 화면이
멈추는 현상 조사·수정

사용자 리포트: 행을 드래그해서 이동하거나 체크박스를 다수 선택할 때
화면이 멈추는 현상이 자주 발생한다는 것 — 원인 조사 요청.

**1) 체크박스 다수 선택 후 이동 — 확인된 진짜 원인(O(N²)), 수정 완료**:
`pages/admin.py`의 `_move_selected()`(위로/아래로 버튼 처리)가 체크된
각 행의 "현재 위치"를 찾을 때마다 `rows` 전체를 처음부터 다시 훑는
`_current_index()`를 썼다 — 이게 `sorted(..., key=_current_index)`(체크한
행 수만큼 호출)와 그 아래 `for path in ordered:` 루프(또 체크한 행
수만큼 호출) 두 곳에서 쓰여, 사실상 O(N²)이었다. 직접 측정: 2,000행을
"전체 선택" 후 "위로" 한 번 누르면 **3.08초** 걸림(5,000행이면 이론상
20초 안팎) — 요청 중엔 Flask/gunicorn 워커가 그 계산에 묶여 있어 사용자
입장에서는 "화면이 멈춘 것"처럼 보인다. 수정: 행 경로별 "현재 인덱스"를
한 번만 사전(`index_of`)에 담아두고, 인접 행끼리 자리를 바꿀 때마다 그
두 행의 인덱스만 O(1)로 갱신하도록 바꿔 전체를 O(N)으로 낮췄다 — 같은
2,000행 테스트가 **0.0085초**로 줄어듦(약 360배). 기존과 똑같은
입력(하위 조직 포함 여부, 경계에서 막히는 것 등)에 대해 동일한 결과를
내는지 여러 시나리오로 대조 확인.

**2) 드래그 재정렬 — 실제로 발견한 별개의 진짜 버그(off-by-one), 수정
완료**: `assets/team_refer_grid.js`의 `trIndex(tr)`이
`Array.prototype.indexOf.call(tr.parentElement.children, tr)`로 DOM 상의
위치를 그대로 배열 인덱스로 썼는데, **dash_table이 헤더 행도 데이터
행과 같은 `<tbody>` 안에 `<tr>`로 넣어둔다는 사실을 놓치고 있었다**
(헤더 행은 `<th>`만 있고 `<td>`가 하나도 없음 — 실제 DOM을 열어 직접
확인). 그래서 모든 데이터 행의 DOM 인덱스가 실제 데이터 배열 인덱스보다
항상 1 커서, 드래그로 A 행을 B 위에 놓으면 실제로는 B가 아니라 B
"다음" 행이 옮겨지는 오작동이 있었다(재현: 2번째 행을 1번째 행 위로
끌어놓으면 기대와 다르게 3번째 행이 끼어드는 것을 실제로 확인). 이건
"화면이 멈추는" 것과는 결이 다른 버그(정지가 아니라 오동작)지만, 사용자
입장에서는 "드래그해도 원하는 대로 안 움직인다"→ 여러 번 다시
시도·재확인하게 만들어 "화면이 이상하다/멈춘 것 같다"는 인상으로
이어졌을 가능성이 있다. 수정: `trIndex()`가 `<td>`가 있는(=진짜 데이터)
행만 세도록 필터링 — 헤더 행(`<td>` 없음)은 인덱스 계산에서 자동으로
제외된다. 200행 데이터로 처음 두 행 드래그(정확히 서로 자리 교체됨)와
중간 지점(50번째↔49번째, 인접한 다른 조직코드 값들도 함께 올바르게
재배당됨) 양쪽 다 정확한 결과로 재현·확인.

**3) 드래그 시 화면 버벅임 — 추가로 정리한 중복 작업(성능 개선,
확정적 수치 측정은 못 함)**: `window.__teamReferOnDataChange`(그리드
data/sort_by가 바뀔 때마다 호출)가 매번 `requestAnimationFrame`으로
`markRowsDraggable(wrap)`을 다시 불러 전체 행을 훑으며 `draggable=true`
를 다시 걸고 있었는데, 이미 `setupRowDrag()`가 등록한 `rowObserver`
(MutationObserver)가 새로 생기는 `<tr>`마다 자동으로 같은 일을 하고
있어 사실상 매번 중복 작업이었다(체크박스 이동/삭제, 드래그 재정렬,
정렬 등 data가 바뀌는 모든 조작마다 전체 행을 두 번씩 훑는 셈). 최초
1회(옵저버가 아직 없어 기존 행을 못 잡아준 시점)에만 명시적으로 부르고
그 이후에는 옵저버에게 맡기도록 정리 — 행이 많을수록(수백~수천 행)
누적되는 중복 DOM 순회를 줄인다. 이 항목은 위 두 가지처럼 정확한
전/후 수치로 확정하지는 못했다(브라우저 렌더링 성능 특성상 정밀 측정이
어려움) — 논리적으로 명백한 중복 제거이며 회귀는 없음을 확인(제거 후에도
데이터 변경 후 모든 행이 여전히 draggable로 정상 마킹되는 것 확인).

**검증**: 200행 합성 데이터로 실제 서버(임시 admin 계정) + Playwright
확인 — (1) 전체 선택 후 위로 이동이 정상 동작(경계에서 막힘 안내까지),
(2) 드래그로 첫 두 행이 정확히 자리를 바꾸는 것, (3) 드래그로 중간
지점(50번째 행을 49번째 위로)도 정확히 바뀌고 조직코드도 올바르게
재배당되는 것, (4) 데이터 변경(이동) 후에도 모든 데이터 행이 여전히
draggable 속성을 갖고 있는 것(중복 제거가 정상 동작을 깨지 않음),
(5) 콘솔/페이지 에러 0건까지 확인. `_move_selected()`는 별도로 단위
테스트(N=50~5000)로 기존 구현과 동일한 결과·큰 속도 차이를 직접 측정.
`python3 -m py_compile`/`python3 -c "import app"` 통과. 테스트 계정·
`data/processed/team_refer.csv`는 검증 후 삭제, `config/users.json`은
원본과 diff 없음 재확인.

**미검증**: 실제 운영 데이터 규모(실제 조직 단위 수)가 정확히 얼마인지
확인하지 못해 O(N²) 버그가 실사용에서 정확히 몇 초짜리 "멈춤"으로
느껴졌는지는 추정만 가능함(수백~수천 단위였다면 리포트된 "자주
발생"이라는 체감과 잘 맞음). 드래그 off-by-one 버그가 이번 세션에서
새로 생긴 회귀인지, 애초에 2026-09-14 드래그 기능 도입 때부터 있었던
...
## 2026-09-17: 직군 예외자 탭도 dash_table → dash-ag-grid 전환

바로 위 항목(팀/리더 참조)과 같은 이유로, 사용자가 "직군 예외자 탭도
진행해줘"라고 요청해 이어서 전환했다. 이 탭은 하위 조직 계층이 없는
평면 목록(사원번호/성명/직군예외 3개 컬럼)이라 팀/리더 참조보다
훨씬 단순하다 — `_row_path`/`_subtree_range`(cascade 삭제)는 애초에
필요 없었다.

**변경 내용**: `_exception_job_function_tab()`의 `dash_table.DataTable`을
`dag.AgGrid`로 교체. `getRowId`용 `_rid`는 팀/리더 참조에서 만든
`_ensure_rid()`를 그대로 재사용(범용 헬퍼라 두 탭이 공유). 'No.' 컬럼도
팀/리더 참조와 동일한 이유로 `valueGetter`가 아니라 `_renumbered()`로
파이썬에서 직접 재번호(그 시행착오를 이미 겪었으므로 처음부터 이
방식으로 구현). 헤더 클릭 정렬/컬럼 리사이즈/툴팁도 AG Grid 기본
제공으로 대체.

**이 탭만의 차이 — 삭제 방식 변경**: 예전엔 `row_deletable=True`로
행별 × 버튼이 있었는데(팀/리더 참조처럼 "선택 삭제" 버튼이 원래
없었음 — 계층이 없어 일괄 삭제 자체가 필요 없었기 때문), AG Grid
Community엔 그 기본 UI가 없어 **체크박스 선택 + "선택 삭제" 버튼을
새로 추가**했다(헤더 전체선택 체크박스 포함, 팀/리더 참조와 동일한
`dashGridOptions`). 행 1개만 지우려면 그 행 하나만 체크하면 된다 —
기능이 없어진 게 아니라 UI 방식만 바뀐 것.

**검증**: 실제 서버(임시 admin 계정, 사원번호/성명/직군예외 합성
데이터 2행) + Playwright로 확인 — 텍스트 셀 편집, 행 추가 후 값 채워
넣기, 체크박스 선택 후 "선택 삭제"로 정확히 그 행만 삭제(cascade
없음 확인), No. 재번호, "저장" 후 CSV에 정확히 반영. 콘솔/페이지 에러
0건. 테스트 계정·`data/processed/exception_job_function.csv`는 검증 후
삭제, `config/users.json`은 원본과 diff 없음 재확인.

**검증 중 발견한 무관한 기존 버그(이번 전환과 무관, 수정하지 않음)**:
`exception_job_function_store.list_editable_rows()`가 빈 문자열 셀을
CSV에 저장한 뒤 다시 불러올 때 `'nan'`이라는 문자열로 잘못 표시되는
문제를 발견했다(직군예외처럼 비워둘 수 있는 컬럼에서 재현). 원인은
`str(row.get(eng, '') or '')`에서 pandas가 빈 문자열을 CSV에 썼다가
다시 읽을 때 `NaN`(float)으로 해석하는데, `NaN`이 파이썬에서 truthy라
`NaN or ''`가 `NaN`으로 평가되고 그걸 `str()`로 감싸 `'nan'`이 되는
것 — 격리된 pandas 재현으로 확인(AG Grid나 dash_table과 무관, 이
...
## 2026-09-17: 팀/리더 참조 — "맨 위로"/"맨 아래로" 버튼 추가

사용자 요청: 체크한 행을 한 칸씩 옮기는 "위로"/"아래로"만 있어서,
멀리 떨어진 곳으로 옮기려면 여러 번 눌러야 하는 불편이 있었다 —
그리드 맨 위/맨 아래로 한 번에 보내는 버튼을 추가.

**구현**: `_move_selected()`(한 칸씩 스왑)와 별개로 `_move_to_edge
(rows, selected_idx, edge)` 헬퍼를 새로 추가 — 체크한 행들을 배열에서
뽑아내(`picked`) 나머지(`rest`)와 합치되, `edge='top'`이면 앞에,
`'bottom'`이면 뒤에 붙인다(둘 다 안정 분할 — `picked`끼리, `rest`
끼리 각각 원래 순서 그대로 유지). "위로"/"아래로"와 마찬가지로
상위부서 경계나 하위 조직 여부와 무관하게 체크한 행만 개별적으로
옮긴다(2026-09-16에 확정된 기존 이동 방침을 그대로 따름 — 하위 조직을
묶어서 옮기지 않음). `team_refer_move_selected` 콜백 하나에 버튼 4개
(맨 위로/위로/아래로/맨 아래로)를 모두 물려 `ctx.triggered_id`로
분기하도록 합쳤다(콜백을 4개로 쪼개는 대신 — Output이 같은 콜백을
여러 개 두면 `allow_duplicate` 관리가 번거로워짐).

**검증**: 실제 서버(임시 admin 계정, 평면 목록 5행 합성 데이터) +
Playwright로 확인 — 체크한 1개 행이 "맨 위로"로 정확히 맨 앞으로,
이어서 다른 2개 행이 "맨 아래로"로 정확히 맨 뒤로 이동(둘 다 순서
유지 확인), No.가 이동 후에도 1..5로 정확히 재번호. 이동한 행은
이동 후에도 계속 체크된 상태로 남는다(기존 "위로"/"아래로"와 동일한
방침 — 첫 테스트 실행에서 이 "선택 유지" 동작을 반영하지 않고 새로
체크만 추가했다가 예상과 다른 결과가 나왔는데, 재현해보니 실제로는
버그가 아니라 이 의도된 동작 때문이었음을 확인). 콘솔/페이지 에러
...
## 2026-09-17: 연구원 프로필 PDF 인쇄 — 회색 배경 누수 수정

사용자 요청: "PDF로 연구원 프로필 인쇄시 모든 화면에 대해서 흰
배경으로 바꿀 수 있을까?" — 실제로 `services/profile_pdf.py`(A4 인쇄
콘텐츠를 헤드리스 브라우저로 PDF 캡처하는 서버사이드 함수)를 그대로
호출해 PDF를 직접 생성하고 `pdftoppm`으로 이미지 변환해 눈으로 확인한
결과, 1페이지 하단(콘텐츠가 끝난 뒤 남는 빈 공간)이 흰색이 아니라
옅은 회색(`#f5f5f5`)으로 찍히는 문제를 재현했다.

**원인**: `app.py`의 `app.layout` 최상위 `Div`가 인라인 스타일로
`backgroundColor: '#f5f5f5'`(화면용 배경, `--gs-bg`)를 갖고 있다.
`assets/custom.css`에 이미 `body`는 인쇄 시 흰색으로 강제하는 규칙이
있었지만(2026-09-16, "Page 배경색으로 옅은 회색이 사용되고 있는데
이걸 제외하면 좋겠어"), 이 `Div`는 그 규칙의 대상이 아니었다 — 인쇄
콘텐츠 안의 흰 박스(`.print-box` 등) 사이 여백이나 페이지 하단처럼
콘텐츠가 이 `Div`를 완전히 덮지 못하는 자리마다 그 회색이 그대로
비쳐 보였다. Playwright로 실제 DOM을 순회해 배경색이 정확히 어느
요소에서 새어 나오는지(`getComputedStyle` 전수 조사) 격리 확인해
찾아냈다.

**수정**: `assets/custom.css`의 기존 `#react-entry-point > div {
min-height: 0 !important; }` 규칙(2026-09-03, 인쇄 시 빈 페이지 한 장
더 붙는 문제를 막으려고 이미 이 요소를 타깃하고 있었음)에
`background-color: #fff !important;`를 추가만 했다 — 새 선택자를
만들지 않고 기존에 정확히 같은 요소를 겨냥하던 규칙에 얹었다. 이
규칙은 `@media print` 전역이라 이 앱의 모든 인쇄 화면(연구원 프로필
A4, 조직별 비교 A3)에 공통 적용되지만, "인쇄 시엔 화면 배경을 흰색
으로" 라는 기존 `body` 규칙과 동일한 정책의 연장이라 범위를 좁힐
이유가 없었다.

**검증**: `services/profile_pdf.py`의 `render_profile_pdf()`를 그대로
호출해 실제 PDF를 생성 → `pdftoppm`으로 페이지별 PNG 변환 → 수정 전
(회색 누수 재현) / 수정 후(완전히 흰 배경) 비교 확인. 조직별 비교
(A3) 페이지도 같은 인쇄 모드로 렌더링해 배경이 여전히 흰색인 것
확인(이 세션의 샘플 데이터로는 해당 화면 자체가 "준비 중" 상태라
내용 렌더링까지는 확인 못 함 — 다만 배경색 자체는 문제없음을 확인).
테스트 계정은 검증 후 삭제, `config/users.json`은 원본과 diff 없음
재확인. 순수 CSS 한 줄 추가라 기능적 리스크 없음.

## 2026-09-18: 팀/리더 참조 — 엑셀 드래그 복사 → 그리드 붙여넣기 직접 구현

사용자 요청: "팀/리더 참조의 테이블에 엑셀에서 드래그한 행렬의 값을
복사해서 붙여넣기 할 수 있을까?" — 격리된 프로브 앱으로 먼저 확인한
결과, **AG Grid Community에는 클립보드 붙여넣기 기능이 아예 없다**
(단일 셀 값 하나도 Ctrl+V로 안 들어감 — 여러 셀/범위 붙여넣기만
막힌 게 아니라 클립보드 기능 자체가 Enterprise 전용). `enable
EnterpriseModules=True`로 켜면 정확히 동작하지만, 라이선스 키 없이는
브라우저 콘솔에 "License Key Not Found — trial" 경고가 계속 뜨고
(화면에 워터마크는 없었지만 무기한 사용은 라이선스 위반), 애초에
AG Grid로 옮긴 이유(무료·MIT 라이선스로 사내에서 제약 없이 쓰기)와
정면으로 상충돼 이 방식은 채택하지 않았다. 사용자가 "직접 구현으로
진행해줘"라고 확정해 아래처럼 구현했다.

**구현 방식**: AG Grid의 내장 클립보드 서비스를 아예 쓰지 않고,
브라우저 표준 `paste` 이벤트를 `#team-refer-grid-wrap`에서 직접
가로챈다(`assets/team_refer_grid.js`). 동작:
1. 셀이 실제로 편집 중(더블클릭/F2로 `<input>` 표시 중)이면 아무것도
   하지 않고 브라우저 기본 동작에 맡긴다 — 그 입력창에 값 하나만
   정상적으로 붙여넣어진다(엑셀에서 셀 하나만 복사해 편집 중인 셀에
   넣는 흔한 경우와 동일하게 동작).
2. 편집 중이 아니고 포커스된 셀만 있으면(엑셀에서 셀을 클릭만 하고
   바로 붙여넣는 것과 같은 상태) `event.clipboardData.getData
   ('text/plain')`로 텍스트를 읽고, 줄바꿈(행)·탭(열)으로 쪼갠다.
3. `api.getAllDisplayedColumns()`에서 편집 가능한 컬럼만 추려 포커스된
   컬럼의 위치를 찾고, 그 지점부터 붙여넣은 열 개수만큼 채운다 — 그리드
   마지막 행/열을 넘어가는 값은 조용히 버린다(새 행을 자동으로 만들지
   않음 — 행 추가는 "행 추가" 버튼의 책임으로 남겨, `_rid`/조직코드
   재배번 같은 다른 콜백과 어긋나지 않게 함).
4. "구분" 컬럼은 `agSelectCellEditor`로 3개 값만 고르게 제한돼 있는데
   (`_WORK_TYPE_OPTIONS`), 붙여넣기는 그 에디터를 거치지 않으므로 JS
   쪽에도 같은 허용 목록을 하드코딩해 목록 밖 값은 붙여넣지 않고
   기존 값을 그대로 둔다(두 곳이 어긋나면 안 되므로 주석으로 명시).
5. 실제 값 반영은 AG Grid의 `rowNode.setDataValue(field, value)`로
   한다 — 격리 테스트로 확인한 바, 이 API는 일반 셀 편집과 동일한
   내부 경로를 타서 dash-ag-grid가 Dash의 `rowData` State를 별도
   콜백 없이도 최신값으로 유지해준다(새 Python 콜백 추가 없이 순수
   클라이언트 스크립트만으로 완결).

**검증**: 실제 서버(임시 admin 계정, 합성 데이터 3행) + Playwright로
확인 — (1) 2행×3열 블록을 포커스된 셀부터 정확히 채우는 것(비공식
소속부서명/구분/1단계부서명 동시에), (2) 그리드 마지막 행/열을
넘어가는 값은 오류 없이 조용히 버려지는 것, (3) "구분"에 허용
목록 밖 값을 붙여넣으면 기존 값이 그대로 유지되는 것, (4) 셀이
편집 중일 때는 이 로직이 개입하지 않고 브라우저 기본 붙여넣기가
정상 동작하는 것, (5) 붙여넣은 값이 "저장" 후 실제 CSV에 정확히
반영되는 것까지 확인. 콘솔/페이지 에러 0건. 테스트 계정·
`data/processed/team_refer.csv`·엑셀 export는 검증 후 삭제,
`config/users.json`은 원본과 diff 없음 재확인.

**한계**: 그리드에 이미 있는 행/열 범위 안에서만 채워진다 — 붙여넣을
블록이 마지막 행보다 아래로 넘치면 그 부분은 버려진다(새 행을 먼저
"행 추가"로 만들어둔 뒤 붙여넣어야 함). 셀 범위를 마우스로 드래그해
선택하는 기능(AG Grid의 "Range Selection")은 여전히 없다 — 이건
이번 요청 범위 밖이라 다루지 않았다(붙여넣기 자체는 지금처럼 셀
하나만 클릭해 포커스해도 충분).

## 2026-09-18: 팀/리더 참조·직군 예외자 표 — 헤더/셀 전부 가운데 정렬

사용자 요청: "팀/리더 참조, 직군 예외자 테이블도 모두 가운데 정렬로
바꿔줘"(사용자/권한 관리 표를 가운데 정렬한 것과 같은 요청). 이
두 표는 AG Grid로 마이그레이션하면서(2026-09-17) 예전 dash_table의
`style_cell={'textAlign': 'center'}` / `style_header={'textAlign':
'center'}` 설정을 그대로 옮기지 않아 기본값(좌측 정렬)으로 돌아가
있었다.

**수정**: 두 그리드의 `defaultColDef`에 `cellStyle: {'textAlign':
'center'}`를 추가해 셀 내용을 가운데 정렬했다(AG Grid는 컬럼마다
따로 지정할 필요 없이 `defaultColDef`로 전체에 한 번에 적용됨).
헤더는 `cellStyle`이 아니라 flex 컨테이너(`.ag-header-cell-label`)라
`assets/custom.css`의 `.gs-ag-grid .ag-header-cell-label`에
`justify-content: center`를 추가해 맞췄다 — 이 클래스는 이미 두 표
모두에 붙어 있어(2026-09-17 스타일 통일 작업) 별도 선택자 없이 한
번의 CSS 변경으로 둘 다 적용됐다.

**검증**: 실제 서버(임시 admin 계정, 합성 데이터) + Playwright로
확인 — 두 표 모두 `.ag-cell`의 계산된 `text-align`이 `center`,
`.ag-header-cell-label`의 계산된 `justify-content`가 `center`인 것,
스크린샷으로 실제 가운데 정렬된 모습 확인. 셀 편집(더블클릭 → 값
변경)이 스타일 변경 후에도 정상 동작하는 것도 재확인. 테스트
계정·합성 데이터는 검증 후 삭제, `config/users.json`은 원본과 diff
없음 재확인.

## 2026-09-18: team_refer.csv 컬럼 분리 문제 — 조사 결과(코드 변경 없음)

사용자 문의: "서버의 team_refer.csv를 다운받아보니 행의 데이터가
하나의 셀에 다 들어가있는데 원래 이런거야? 예전엔 컬럼별로 구분되어
나왔는데" — CSV를 실제로 다시 생성해 원시 바이트까지 직접 확인한
결과, 이 저장소의 코드(`pipeline/merge_utils.py`의
`write_merged()`)는 지금도 표준적인 콤마 구분·큰따옴표로 감싼 CSV를
만들고 있음을 확인했다(`"org_name_wd","work_type",...` 형태 —
필드마다 정상적으로 콤마로 구분됨). 이 CSV 저장 로직(`quoting=
csv.QUOTE_NONNUMERIC`)은 2026-08-12(`e6fe3a5`)에 도입된 것으로,
이번 세션에서 다룬 AG Grid 전환이나 다른 어떤 변경과도 무관하다 —
`pages/admin.py`/`services/team_refer_store.py`가 이 함수를 호출하는
방식도 그대로다.

파일 자체는 정상이라, "예전엔 컬럼별로 나왔는데 지금은 한 셀에
다 들어간다"는 증상은 파일을 여는 프로그램(엑셀 등)의 지역별
목록 구분자 설정이 콤마(,)가 아닌 다른 문자(세미콜론 등)로 돼
있을 때 흔히 나타나는 현상일 가능성이 높다(윈도우/엑셀이 .csv
더블클릭 시 시스템 "목록 구분 기호" 설정을 따르는데, 이게 콤마가
아니면 전체 줄이 한 셀로 들어가 보임 — 실제로는 파일이 깨진 게
아니라 여는 쪽 설정 문제인 경우가 많음). 사용자에게 어떤
프로그램으로 열었는지, 원본 파일의 첫 줄을 텍스트 편집기로 봤을
때 실제로 콤마가 있는지 확인해달라고 요청 — 코드 변경 없이 조사만
하고 종료.

## 2026-09-18 (2): docs/데이터_계보_등록부.xlsx — 헤더 단위 상세 매핑 시트 신설
+ 누락 원천 3건 등록 + 정확도 정정 2건

사용자 요청: 원본 파일의 어떤 헤더가 전처리 파일의 어떤 헤더로 어떤 방식
으로 처리되는지(각 원본 파일이 실제로 쓰는 모든 헤더 기준) 상세히
정리해, 처리 스크립트·누적 방식 등 기존 등록부 내용도 전부 포함해서
갱신해달라는 것. `pipeline/sources.py` 전체(27개 원천 항목)와 등록부의
기존 27행을 대조해 실제 누락은 3건(mapping_job_function/
exception_job_function/evaluation_exception, 전부 2026-09-04~09-09에
새로 추가된 원천)뿐임을 확인 — 그 외 항목(어학/근무 경력/팀 리더 참조
포함)은 이미 등록부에 반영돼 있었다.

**신규 시트 "헤더 매핑 상세"**("원천 데이터 등록부" 다음, "LLM 파생 분석
산출물" 앞에 배치): 원천 30개(원천 정제 23개 + 원본 통과 4개 + 신규 3개)
전체를 스크립트 단위 구획으로 나눠, 각 구획 안에 "매핑"(원본 헤더→출력
컬럼 직접·변환 매핑)/"파생"(파생·계산 컬럼, 원본 헤더 1개로 안 끝나는
것)/"규칙"(행 필터링·정렬·중복제거·병합 방식) 3종 행을 색으로 구분해
나열 — 총 319행(구획 헤더 30 + 상세 행 약 280 + 교차참고 노트 5).
데이터는 이번 세션에서 직접 읽은 4개 스크립트(process_mapping_job_
function.py/process_exception_job_function.py/process_evaluation_
exception.py/process_team_refer.py+team_hierarchy.py)와, 이전 세션에서
남은 ~22개 process_*.py를 5개 배치로 나눠 돌린 백그라운드 조사 에이전트
결과(각 에이전트가 소스 코드를 직접 읽고 원본 헤더 1개 단위까지 정확히
추출한 보고서, `SubagentHandback` 메시지로 온전히 회수해 재사용)를
합쳐서 구성했다 — 모든 헤더명·변환 로직은 코드에서 직접 확인된 내용만
반영(추측 없음).

**기존 등록부("원천 데이터 등록부") 정정 2건**:
- No.23 근무 경력: 누적 방식이 여전히 "그룹 교체"로 적혀 있었는데, 실제
  코드는 2026-09-02부터 자연키 upsert(researcher_id+company_name+
  work_start_date)로 전환돼 있었다 — 최초 도입 시점 설명이 그대로 남아
  코드 변경을 놓친 사례. 자연키/그룹키 컬럼도 함께 정정.
- No.21 팀/리더 참조: 웹 업로드 지원 방식을 "exact"(고정 파일명만)에서
  2026-09-11에 이미 "wildcard(xlsx 또는 csv)"로 바뀐 것을 반영, 아울러
  2026-09-11 3단계 인텔이크 스키마 전환(1/2/3단계부서명 전체 경로 입력 +
  dep_id/upper_dep_id/team_layer 자동 계산) 사실을 누적 방식 설명에 추가.

**신규 원천 3행 추가**(No.28~30, 노란색 강조): 직무_직군_맵핑/직군
예외자/평가표기 예외자 — 각각 `pipeline/sources.py`/`config/auth_
config.py`(AI 검색 노출 범위)/`services/web_pipeline_runner.py`
(웹 업로드 등록)/`pipeline/load_to_db.py`(DB 반영 대상)를 실제로 조회해
정확한 값(웹 업로드 지원 방식, DB 반영 Y/N, AI 검색 노출 여부)을 채웠다.

**LLM 파생 분석 산출물 시트**: process_project_search.py("과제별 유사
기업/학계 탐색", project_searched_list.csv)가 빠져 있던 것을 4번째
행으로 추가 — process_project_expertise.py와 문서 요약 캐시를 공유하지만
앱 화면 어디서도 직접 조회하지 않는 순수 배치 산출물임을 명시.

**표지 시트**: 기준일 2026-09-18로 갱신, 최근 반영 문구를 이번 변경
4가지(신규 원천 3건/헤더 매핑 상세 시트 신설/근무경력 누적방식 정정/
팀·리더참조 웹업로드 방식 갱신) 요약으로 교체.

**검증**: 스타일은 기존 파일에서 직접 추출한 값(Arial 10pt, 헤더
네이비(001F4E78)/흰 글씨, 데이터 얇은 회색 테두리, No.열만 가운데정렬
나머지 좌상단+wrap, 노란 하이라이트 00FFF2CC)을 그대로 재사용해 새 행/
새 시트에 일관되게 적용. 수식이 전혀 없는 순수 문서 시트라 `recalc.py`는
불필요(xlsx 스킬 가이드 확인). 저장 후 openpyxl로 재오픈해 5개 시트·
전체 dimension·시트 순서(표지→원천 데이터 등록부→헤더 매핑 상세→LLM
...
## 2026-09-16: 팀/리더 참조 — 조직코드(dep_code) 문자열 정렬 버그 수정
(0-패딩) + 숨김 조직 순서 정리

사용자가 운영 서버에서 실제로 겪은 여러 문제를 순서대로 진단·해결한
세션. 요약:
1. `data/processed/team_refer.csv`가 CP949로 덮어써져 있어 `save_snapshot()`
   의 `read_existing()`이 매번 `UnicodeDecodeError`로 500 에러 — 저장이
   전부 실패하고 있었다(사용자 신고: "저장/DB반영까지 완료했는데 다른
   탭 갔다 오면 원래대로"). 서버 로그 traceback을 단계적으로 요청해
   정확한 실패 지점(`merge_utils.read_existing()`의 `pd.read_csv(...,
   encoding='utf-8-sig')`)을 특정한 뒤, 원본 바이트를 직접 열어
   cp949/euc-kr로는 정상 디코딩됨을 확인 — 사용자가 최근에 그 파일을
   직접 덮어쓴 사실을 확인해줘 원인 확정. 백업 후 cp949→utf-8-sig로
   재인코딩(데이터 손실 없이 형식만 교정)해 해결.
2. "엑셀 파일로 한번에 반영"(대량 업로드) 후 과거 데이터가 함께 보이는
   문제 — 코드 버그가 아니라, 대량 업로드(`pipeline/process_team_refer.py`
   의 `process()`)는 `team_refer.csv`만 갱신하고 DB에는 반영하지 않는
   구조인데(DB 반영은 `pages/admin.py`의 별도 "DB 반영" 버튼,
   `run_team_refer_db_load()` 몫), 화면은 DB를 우선 읽어서 업로드
   이전 DB 스냅샷이 계속 보이고 있었던 것 — 기존에 정확히 이 상황을
   위해 만들어둔 "DB 반영" 버튼을 누르는 것으로 해결(코드 변경 없음).
3. 붙여넣기(`assets/team_refer_grid.js`)가 안 되는 것처럼 보였던 건
   일시적인 상태였고(재확인 시 정상 동작) — 진단 과정에서 클립보드
   원본 텍스트를 콘솔에 직접 찍어보는 방법(`clipboardData.getData
   ('text/plain')`을 `JSON.stringify`로 로그)을 사용해 탭/줄바꿈이
   정상적으로 들어있음을 확인했다.

**진짜 남은 버그(이번에 코드로 수정)**: 순서를 조정(위로/아래로/맨위로/
맨아래로 버튼)한 뒤 저장·DB반영해도 다른 페이지 갔다 오면 순서가
다시 흐트러지는 문제 — 사용자가 직접 원인을 정확히 짚어냈다: 형제
정렬 기준인 조직코드(dep_code)가 `pipeline/rd_specialist_markdown.py`의
`build_org_tree()`에서 **문자열**로 비교되는데(`sort(key=lambda n:
n.get('dep_code',''))`), `pages/admin.py`의 `_renumber_dep_codes()`가
패딩 없이 `str(i)`(1, 2, ..., 10, 11...)로 매겨서 "10"이 "2"보다 문자상
앞서 버리는 것이었다(`team_hierarchy.py`의 신규 조직 기본값 채움은
이미 `f'{i:04d}'`로 4자리 패딩하고 있어 두 경로가 서로 다른 규칙을
쓰고 있었음).

**해결(사용자 확정 방향 — 서버 저장 시점 한 곳으로 통합)**:
`services/team_refer_store.py`에 `_renumber_dep_codes()`를 새로 추가해
`save_snapshot()` 맨 앞에서 무조건 호출하도록 했다:
- 비공식소속부서명이 있는(화면에 보이는) 행은 화면 순서 그대로 `0001`
  부터 4자리 패딩으로 다시 매긴다.
- 비공식소속부서명이 빈(관리자 화면에서 순서를 확인/조정할 수 없는,
  `pages/admin.py`의 `_split_hidden_rows()`로 숨겨지는) 행은 `9999`부터
  거꾸로 매겨, 보이는 조직의 범위(0001~)와 절대 겹치지 않고 항상
  형제들 중 맨 뒤로 밀려나게 했다(사용자 제안 그대로).
- 저장할 때마다 무조건 다시 매기므로, 예전에 패딩 없이 저장된 조직코드도
  다음 저장 한 번으로 자동 정리된다 — 별도 마이그레이션 스크립트가
  필요 없다. `pages/admin.py`의 버튼별 클라이언트 사이드 `_renumber_
  dep_codes()`(패딩 없는 `str(i)`, 이동 직후 화면에 즉시 보여주는 용도)는
  그대로 남겨뒀다 — 최종적으로 저장되는 값은 항상 서버 쪽에서 다시
  정리되므로 화면상 임시 표시 값과 무관하게 항상 올바르게 저장된다.

**검증**: `_renumber_dep_codes()` 단위 테스트로 (1) 보이는/숨김 행이
섞여 있어도 보이는 행끼리는 화면 순서 그대로 0001~ 매겨지고 숨김
행은 항상 9999 쪽으로 밀리는 것, (2) 형제 12개(패딩 없으면 "10"이
"2"보다 앞서던 상황)를 문자열 정렬해도 원래 순서와 정확히 일치하는
것을 확인. `pipeline.process_team_refer.build_rows_from_records()`
전체 파이프라인에 12개 형제 파트를 흘려보내 `dep_code`가 0001~0014로
정확히 이어지는 것(10파트~12파트가 9파트 뒤로, 2파트 앞이 아니라)까지
end-to-end로 확인. `python3 -m py_compile services/team_refer_store.py`
통과.

**미검증**: 실제 운영 서버에서 화면으로 재현(위로/아래로 이동 → 저장 →
DB반영 → 다른 탭 갔다 옴)까지는 이 세션 밖(사용자가 직접 확인)이라
실행 결과는 다음 확인이 필요하다.

## 2026-09-16 (2): 관리자 화면 — 안내 문구 정리

사용자 요청으로 `pages/admin.py`의 "i" 안내 문구(dbc.Alert/작은 회색
설명 텍스트) 5곳을 삭제하고 1곳을 교체했다 — 전부 화면 동작에는
영향 없는 텍스트만 지운 변경.

**삭제**(팀/리더 참조 탭 4곳 + 직군 예외자 탭 1곳):
- "1단계부서명은 필수입니다... 데이터는 그대로 보존되며 저장 시 함께
  반영됩니다." (숨김 행 개수 안내 포함 전체 Alert)
- "입력 날짜" 아래 "기본값은 오늘 — 과거 데이터를 소급 입력할 때만
  바꾸세요."
- 행추가/저장/엑셀 다운로드/DB반영 버튼 아래 "엑셀 다운로드는 현재
  화면의 편집 내용이... (화면 편집 중인 내용 아님)."
- 맨위로/위로/아래로/맨아래로/선택삭제 버튼 아래 "왼쪽 체크박스로...
  항상 자동으로 매겨집니다."
- 직군 예외자 탭 "사원번호가 비어... (DS 직군은 그대로 유지됩니다)."

(직군 예외자 탭의 "왼쪽 체크박스로... 저장된 최신 값을 내려받습니다"
문구는 삭제 대상이 아니라 그대로 남겨뒀다 — 사용자 요청이 "사원번호가
비어..." 문구 1개만 지정했음.)

**교체**(데이터 업데이트 탭): "엑셀 업로드 시 복호화(일반문서로 변환)
후 업로드 가능합니다. '전체 업데이트'는 파일이 업로드된 항목만
실행합니다."를 사용자가 요청한 문구("엑셀 업로드 시 복호화(일반문서로
변환) 후 업로드 가능, 전체 업데이트는 파일이 업로드 된 항목만 실행")로
교체 — 내용은 거의 동일하고 문장부호·어미만 사용자 지정 표현으로
맞췄다.

**검증**: `python3 -m py_compile pages/admin.py` 통과, 지운 문구가
전부 사라지고 남겨야 할 문구(직군 예외자의 다른 안내, 데이터 업데이트
탭 Alert 자체)는 그대로 있는지 grep으로 재확인.

## 2026-09-16 (3): 보유 전문성 — 소개 문구 정리 + 조직도의 구조용
(비공식소속부서명 없는) 노드 라벨 숨김

**1. 소개 문구**: `pipeline/process_researcher_expertise.py`의 "연구원
전문성 콘솔" 사이드바 태그라인을 사용자가 지정한 형태(2줄 불릿)로
교체 — "학력·과제이력·직무이력·기술·논문·특허 종합 분석 (+ 유사 연구원
매칭) (R&D Talent Profiling Agent)" 한 문장을, "- 학력(전공), 과제이력,
직무이력, 논문, 특허, 업무목표 등 기술이력 기반 종합 분석" / "- 유사
연구원 매칭(R&D Talent Profiling Agent)" 두 줄(`<br>`)로 분리했다.
기존 동작(두 번째 줄은 유사 연구원 데이터가 실제로 있을 때만 표시,
`similar_by_id` 조건)은 그대로 유지 — 이번 요청은 문구·줄바꿈만
바꾸는 것이라 조건 자체를 없앨 이유가 없었다.

**2. 조직도 구조용 노드 숨김**: 사용자 질문("팀/리더 참조에서 숨긴
비공식소속부서명이 없는 부서가 보유 전문성 조직도에서는 나오고
있는데 여기서도 숨길 수 있어?")에 가능하다고 확인하고 바로 적용했다.
`pipeline/rd_specialist_markdown.py`의 `org_tree_html()`(보유
전문성·유사 연구원 리포트 사이드바 조직도를 만드는 공용 함수 — 두
리포트 모두 이 함수 하나를 공유해 한 번의 수정으로 둘 다 반영됨)에
이미 있던 "자신에게도 하위 조직에도 콘텐츠가 없으면 생략" 규칙만으로는
부족했다 — 비공식소속부서명이 없는 노드는 실제 배정이 없는 "구조용
상위 노드"라 자기 콘텐츠는 없지만, 그 밑에 실제 배정된 하위 조직이
있으면(`children_html` 존재) 지금까지는 그대로 라벨(`<details>/
<summary>`)이 보였다. `_node_html()`에 `org_name_wd`가 빈 노드는
`<details>/<summary>` 래퍼 자체를 생략하고 `extra + children_html`을
바로 반환하도록 분기를 추가 — 그 노드의 라벨(이름)만 안 보이고, 하위
조직·연구원은 한 단계 얕아진 채(부모 조직 바로 밑으로) 그대로 남는다.
`.org-node-body`(들여쓰기용 div) 래퍼도 이 경우엔 씌우지 않아 시각적으로
불필요한 추가 들여쓰기가 생기지 않는다.

**검증**: `python3 -m py_compile pipeline/process_researcher_expertise.py
pipeline/rd_specialist_markdown.py` 통과. `org_tree_html()`에 합성
트리(비공식소속부서명 없는 "AI사업부" 상위 노드 + 그 밑에 실제 배정된
"AI팀")를 직접 넣어 렌더링한 HTML을 확인 — "AI사업부" 라벨은 결과에
없고 "AI팀"은 정상적으로(최상위로 끌어올려져) 렌더링됨을 확인했다.

## 2026-09-16 (4): Job Market — 추천 결과 A/B 안내 문구 정리

사용자 요청으로 `pages/job_market.py`의 추천 결과 "i" 안내 아이콘
툴팁 문구를 교체했다. A/B 배지가 각각 어떤 기준의 유사도인지 설명하는
`_SCORE_A_DESC`/`_SCORE_B_DESC`를 기술적 설명("그 과제 자체의 분석
문서(컨플루언스 요약) 임베딩과..." 등)에서 사용자가 지정한 쉬운
문구("과제에서 필요한 기술과 해당 연구원의 보유 전문성 유사도 기반" /
"과제 참여중인 인력의 보유 전문성과 해당 연구원의 보유 전문성 유사도
기반")로 바꿨고, B의 라벨명도 "배정 인력 전문성 기반"에서 사용자가
지정한 "해당 과제에 속한 인력 전문성 기반"으로 바꿨다. 결과 상단
요약 문구(`_render_result()`의 `header_line`, "(A: 과제 분석 기반,
B: 배정 인력 전문성 기반...)")도 같은 B 라벨로 통일해 툴팁과 어긋나지
않게 했다 — 요청 범위 밖이지만 같은 화면에 두 가지 다른 표현이 남는
것을 피하기 위함. `_score_badge()`(추천 줄마다 붙는 "A: 83%"/"B: 70%"
배지 자체)는 손대지 않았다 — 사용자가 마지막에 "*A(...), B(...)는
아이콘으로 표시"라고 덧붙인 것도 이미 그렇게(배지 아이콘) 구현돼
있는 현재 상태를 설명한 것으로 판단해 별도 변경은 하지 않았다.
...
## 2026-09-16 (6): 직군 예외자 안내 문구 추가 삭제 + 조직코드
재저장 때마다 번호가 바뀌던 문제 수정(기존 값 존중으로 전환)

**1. 안내 문구**: 직군 예외자 탭에서 "행 추가" 버튼 그룹 아래 "왼쪽
체크박스로... 저장된 최신 값을 내려받습니다." 문구를 삭제했다(팀/리더
참조 쪽은 이미 지난 요청 때 지웠고, 이번엔 직군 예외자 쪽도 지워달라는
요청).

**2. 조직코드가 매 저장마다 바뀌던 문제**: 사용자가 "저장하고 다음번
팀/리더 참조를 들어갔을 때 기존 조직코드 오름차순으로 보여지게 해달라"고
재요청 — 바로 전 세션에서 도입한 `services/team_refer_store.py`의
`_renumber_dep_codes()`가 **저장할 때마다 무조건 화면 순서 그대로 전체
조직코드를 0001부터 다시 매기던 것**이 원인이었다. "조직코드" 컬럼은
실제로 그리드에서 직접 타이핑해 편집 가능한 컬럼이라(`pages/admin.py`의
`column_defs`가 `KOREAN_COLUMNS` 전체를 `editable: True`로 렌더링 —
조직코드도 포함), 이동/추가/삭제 버튼을 전혀 안 눌러도(다른 필드만
고쳐 저장) 매번 무조건 재배정하면 (a) 그 저장 이전에 있던 조직코드
값(관리자가 직접 입력했을 수도 있는 값 포함)이 매번 무의미한 순번으로
덮어써지고 (b) 값 자체가 바뀌어 보이는 것을 사용자가 "번호가 바뀌었다"고
정확히 지적했다.

**수정**: `_renumber_dep_codes()`를 "이미 값이 있으면 그대로 두고
4자리 패딩만 한다, 값이 비어 있는 행(새로 추가된 행 등)에만 현재
보이는 조직 중 가장 큰 값 다음 번호를 새로 매긴다"로 바꿨다. 순서를
바꾸고 싶을 때는 여전히 위로/아래로/맨위로/맨아래로 버튼을 쓰면 되고
— 그 버튼들(`pages/admin.py`의 클라이언트 쪽 같은 이름 함수)이 이미
클릭 시점에 화면 순서 그대로 새 조직코드를 모든 보이는 행에 매겨
넘겨주므로, 서버 쪽은 그 결과(전부 non-empty)를 그대로 존중해서 패딩만
하면 되고 실제로 그렇게 동작한다. 이 김에 클라이언트 쪽 함수도
`str(i)`(패딩 없음)에서 `f'{i:04d}'`로 통일해, 이동 버튼을 누른 직후
화면에 보이는 값과 저장 후 다시 불러온 값이 형식까지 일치하게 했다.
비공식소속부서명이 빈(관리자 화면에 안 보이는) 숨김 조직은 "기존 값
존중"이 애초에 의미가 없어(화면에서 확인/조정 불가) 지난번 결정대로
계속 매번 9999부터 거꾸로 다시 매긴다 — 이 부분은 이번 수정과 무관하게
그대로 유지.

**검증**: `_renumber_dep_codes()` 단위 테스트로 (1) 기존 값(패딩 없음,
갭 있음 — 5/12/33)이 있는 행을 순서 변경 없이 다시 넣으면 패딩만 되고
값 자체는 그대로 유지되는 것, (2) 새로 추가된(조직코드 빈) 행은 기존
최댓값 다음 번호를 받는 것, (3) 숨김 행은 여전히 9999부터 거꾸로
매겨지는 것을 확인. `pipeline.process_team_refer.build_rows_from_records()`
전체 파이프라인에 갭이 있는 기존 값(3, 6, 9, ..., 36)을 흘려보내
`dep_code`가 재배정되지 않고 그 값 그대로(0003, 0006, ..., 0036)
유지되는 것까지 end-to-end로 확인. `python3 -m py_compile pages/admin.py
services/team_refer_store.py` 통과.

## 2026-09-16 (7): 팀/리더 참조 — 조직코드를 "실제 트리 순회 순서"로
다시 매기도록 재설계(3번째 시도, 확정)

바로 전 수정("이미 값 있으면 그대로 존중, 패딩만") 배포 후 사용자가
재확인한 결과, 조직코드가 `0001, 0071~076, 0081~084, 0019~025, 0070,
0094~098`처럼 전혀 오름차순이 아닌 뒤섞인 순서로 나타났다. 사용자
질문("정렬에 dep_id, upper_dep_id과 연관이 있는걸까? 저장할 때를
기준으로 가장 윗행부터 아래행 순으로 0001, 0002, 0003을 매기는 거
맞지?")에 답하며 근본 원인을 재점검했다.

**진단**: (1) 화면에 보이는 조직코드를 위→아래로 그냥 읽으면 오름차순
처럼 안 보이는 게 정상이다 — `build_org_tree()`는 "같은 부모 밑
형제들끼리만" 조직코드로 정렬하지, 전체를 한 줄로 정렬하지 않는다.
부모 그룹이 바뀌면 완전히 다른 숫자대로 넘어가 보인다. (2) 하지만
진짜 버그도 있었다 — 2026-09-16 (7) 이전 버전(바로 전전 시도)이 저장
때마다 "화면에 보이는 행 배열 + 숨김 행을 배열 맨 뒤에 이어붙인 순서"
기준으로 번호를 다시 매겼는데, 이건 실제 트리 순회 순서와 다르다.
비공식소속부서명이 없는(숨김) 조직이 어떤 자식들의 부모인 경우, 그
부모는 9999쪽으로 밀려나고 자식들은 낮은 번호를 받아 부모-자식 번호가
완전히 따로 놀았다 — 사용자가 재보고한 뒤섞인 순서의 진짜 원인.
바로 전 수정("이미 값 있으면 존중")은 이 뒤섞인 옛 값을 그대로 보존만
해서 증상을 감췄을 뿐 고치지 못했다.

**해결**: `services/team_refer_store.py`에 `_assign_depth_first_dep_codes()`
를 새로 만들어 `_renumber_dep_codes()`를 대체했다. `build_rows_from_records()`
로 dep_id/upper_dep_id가 계산된 뒤(즉 실제 트리 구조가 확정된 뒤)
`mmd.build_org_tree()`로 트리를 구성하고, 그 트리를 부모→자식→다음
형제 순서로 직접 순회하며 0001부터 새 번호를 매긴다 — 화면(그리고
`org_tree_html()`)이 그리는 순서와 정확히 같은 순서다. 이 방식은
사용자가 원한 "저장 시점에 위→아래 0001,0002,0003"을 실제로 구현하면서
동시에 안정적이다: 트리 구조(부모-자식 관계·형제 상대 순서)가 이번
저장에서 안 바뀌었으면 같은 순회를 거쳐도 항상 같은 값이 나온다(고정점
— 안 바뀐 상대 순서를 1,2,3...으로 다시 매기면 늘 같은 결과), 그래서
순서를 안 바꾸고 다른 필드만 고쳐 저장해도 조직코드가 요동치지 않는다
— 처음 보고됐던 "저장할 때마다 값이 바뀐다"는 불만도 함께 해결된다.
숨김 조직을 9999로 미는 예외 처리는 완전히 삭제했다 — 이제 숨김
조직도 자기 트리 위치에서 자연스럽게 번호를 받아 부모-자식 번호가
어긋나지 않는다(더 단순하고 정확함). `pages/admin.py`의 클라이언트
쪽 `_renumber_dep_codes()`(이동 버튼 클릭 직후 화면에 즉시 보여주는
미리보기 값)는 그대로 남겨뒀다 — 어차피 저장 시점에 서버가 최종값을
다시 매기므로 그 전까지의 미리보기 값과 정확히 같을 필요는 없다.

**부작용(사용자에게 알려야 함)**: 이제 "조직코드" 셀에 직접 값을 타이핑해도
다음 저장 때 트리 위치 기준으로 덮어써진다 — 순서를 바꾸려면 위로/
아래로/맨위로/맨아래로 버튼을 써야 한다. 이건 사용자가 명시적으로
확인한 방향("저장할 때를 기준으로... 매겨주는 거 맞지?")과 일치한다.

**검증**: 사용자가 보고한 것과 동일한 뒤섞인 시나리오(숨김 부모의
조직코드가 9999, 그 자식들은 0071/0072, 손자는 0019/0020)를 그대로
재현해 `_assign_depth_first_dep_codes()`에 흘려보낸 결과 0001(부모)
→0002/0003(자식) →0004/0005(손자)로 정확히 트리 순서대로 재배정됨을
확인. 이 결과를 다시 한 번 더 입력해(구조 변경 없음) 정확히 동일한
값(0001~0005)이 나오는 것으로 안정성(idempotent)도 확인.
`python3 -m py_compile pages/admin.py services/team_refer_store.py` 통과.

## 2026-09-16 (8): 팀/리더 참조 엑셀 일괄 업로드 — 1단계부서명만 있는
고아 루트를 SAIT/종합기술원 밑으로 자동 편입

사용자 요청: intake(scripts/build_team_refer_intake.py 산출물)로 만든
데이터에는 사람 배정이 없어 1단계부서명만 채워지고 2/3단계부서명·
비공식소속부서명이 전부 빈 조직 행이 들어올 수 있는데(예: "AI융합팀"),
그 1단계부서명이 실제 최상위("SAIT"/"종합기술원")가 아니면 겉보기엔
최상위처럼 보여도 실제로는 그 밑에 속하는 조직이다 — "보유 전문성"
조직도가 부모-자식 관계를 올바르게 그리도록, 이런 "고아 루트"의
upper_dep_id를 SAIT/종합기술원의 dep_id로 채워달라는 요청. 구현 전
이해한 내용을 설명하고 확인이 필요한 4가지를 먼저 물어 답을 받았다:
(1) 한 업로드 파일 안에 SAIT/종합기술원이 동시에 존재할 일은 없다(둘
중 실제 있는 쪽 하나만 찾으면 됨), (2) 둘 다 없으면 기존과 동일하게
최상위로 둔다, (3) 엑셀 일괄 업로드 경로에만 적용(관리자 그리드 수동
저장에는 적용하지 않음 — "보유 전문성 조직도 반영 목적"이라 배치
업로드에만 필요), (4) "SAIT"/"종합기술원" 판정은 정확히 일치할 때만.

**구현**: `pipeline/process_team_refer.py`에 `reparent_orphan_roots()`를
새로 추가해 `process()`(엑셀 일괄 업로드)에서 `build_rows_from_records()`
직후 호출한다 — `services.team_refer_store.save_snapshot()`(관리자
그리드 저장)에는 넣지 않아 사용자 확정 범위를 정확히 지켰다. 로직:
`team_layer=='1'`이고 `dep_1st_name`이 "SAIT"/"종합기술원" 중 하나인
행을 찾아(있으면) 그 `dep_id`를 구하고, `team_layer=='1'`이면서
`dep_1st_name`이 그 두 이름이 **아니고** `dep_2nd_name`/`dep_3rd_name`/
`org_name_wd`가 전부 빈 행("고아 루트")들의 `upper_dep_id`를 그 값으로
채운다. SAIT/종합기술원 루트 자체가 이번 파일에 없으면(root_rows가
비어 있으면) 아무것도 바꾸지 않고 그대로 반환한다. `team_layer` 값
자체는 건드리지 않았다 — 부모-자식 관계는 upper_dep_id만으로 결정되고
(dep_id 기반), team_layer는 화면 표시(자기 레벨 이름 선택)에만 쓰여
바꿀 필요가 없었다.

**검증**: (1) SAIT 루트 + 고아 루트("AI융합팀") + 정상 하위조직(AI사업부
→ AI팀, 리프 있음)을 섞은 합성 데이터로 고아 루트만 SAIT 밑으로
편입되고 정상 하위조직의 부모 관계는 그대로인 것, (2) "AI사업부"처럼
2단계 자식(AI팀)을 둔 리프 없는 구조용 1단계 노드도 동일 규칙에 따라
SAIT 밑으로 편입되는 것(그 자식 AI팀의 부모는 여전히 AI사업부로
불변), (3) SAIT/종합기술원 루트가 아예 없으면 아무것도 안 바뀌는 것,
(4) "종합기술원" 루트만 있는 경우도 동일하게 동작하는 것을 각각
확인. `python3 -m py_compile pipeline/process_team_refer.py` 통과.

**미검증**: 실제 intake.py 산출물(진짜 인력현황 원본 기반)로 돌려본 건
...
## 2026-09-17 (11): team_refer.csv 저장 형식 재배치 — "칸 고정 hop" 규칙으로
재정의 + 그 과정에서 발견한 조직도 라벨/그리드 증식 버그 수정

**배경**: 2026-09-17 (앞선 세션)에 도입한 `reshape_storage_columns()`
("own_idx 이상 칸=자기 이름" 규칙)를 사용자가 다시 검토하면서, 2단계
조직(예: "2D")의 2단계부서명 칸도 "부모 이름"(ADDP)으로 채워야 한다는
더 구체적인 요구를 확인했다 — 최종 규칙은 "각 칸은 자기 자신에서 몇
hop 위 조상을 보여줄지가 칸 위치로 고정된다"(3단계 칸=0-hop/자기 자신,
2단계 칸=1-hop/직속 부모, 1단계 칸=2-hop/조부모, 실제 hop 수는
`min(목표 hop, 자기 깊이-1)`로 캡). 예: 2D(깊이 2)는 조부모가 없어
1단계 칸도 1-hop(부모)로 캡돼 1·2단계 칸에 똑같이 "ADDP"가 들어간다.

SAIT 강제 편입 범위(1단계이면서 SAIT/종합기술원이 아닌 orphan 노드만
해당, 이미 하위 조직이 있는 노드는 그대로 둠)는 기존과 동일함을
사용자에게 두 가지 해석(A: orphan만 / B: 전부)을 예시 트리로 만들어
보여주고 A로 확정받았다 — 코드 변경 없음(`reparent_orphan_roots()` 그대로).

**구현**: `pipeline/process_team_refer.py`의 `reshape_storage_columns()`
내부 `_reshape()`를 `min(2-i, depth-1)` hop 계산으로 교체. 추가로
**org_name_wd가 없는 행(경로상으로만 존재하는 조상 전용 노드)은 이제
재배치 대상에서 아예 제외**한다 — 이런 노드는 org_name_wd도 없고
자기 칸이 재배치로 덮이면 원래 경로 텍스트 라벨이 그 어디에도 남지
않아 영구히 사라지기 때문(외부 시스템 요구사항도 애초에 org_name_wd가
있는 행에만 적용되므로 건드리지 않아도 위반이 아님).

**직접 재현해서 찾아낸 버그 2건(이전 세션 구현의 부작용, 이번에 같이 수정)**:
1. **조직도 라벨 오염**: `rd_specialist_markdown.own_level_name()`이 "자기
   team_layer 칸만 읽으면 곧 자기 이름"이라는 전제로 동작했는데,
   reshape 이후 그 전제가 2단계 노드에 더 이상 성립하지 않아(2단계
   노드의 2단계부서명 칸에 부모 이름이 들어감) "2D" 노드의 조직도
   라벨이 "ADDP"로 잘못 표시되는 회귀를 발견. `own_level_name()`이
   `org_name_wd`(reshape가 절대 건드리지 않는 별도 컬럼)를 우선 쓰도록
   수정 — org_name_wd가 없는 행만 기존처럼 자기 칸을 폴백으로 본다.
2. **그리드 재저장 시 조직 트리 증식**: 그리드 저장(`own_path()`)은
   "1/2/3단계 칸에 값이 있으면 그게 전체 경로"라고 해석해 첫 빈 칸에서
   멈추는데, reshape로 3칸이 전부 채워지면 빈 칸이 없어져 "SAIT/SAIT/SAIT"
   같은 값이 "3단계 깊이의 서로 다른 조직"으로 오인된다 — 실제 재현
   결과, 10개 조직을 그리드에서 수정 없이 저장만 다시 눌러도 20행으로
   증식했다(org_name_wd가 빈 유령 행이 계속 생김). `pipeline/team_hierarchy.py`
   `backfill_full_path()`를 수정해, 조상 체인을 걷기 전에 먼저 각 행의
   1/2/3단계부서명 칸을 전부 지우고 새 헬퍼 `_true_own_name()`
   (org_name_wd 우선, 없으면 자기 칸 폴백)으로 자기 칸 하나만 다시 채운
   뒤 조상 이름으로 나머지 빈 칸을 채우도록 바꿨다 — 저장 형식이 어떻게
   재배치돼 있든 항상 "자기 칸=자기 이름, 나머지=조상 이름 또는 공백"
   불변식을 복원한다.

**검증**: 사용자가 제시한 10개 조직 예시(SAIT/ADDP/인사~인사파트2) 전체를
`preview_transform()`으로 돌려 dep_1st/2nd/3rd 값이 예시 표와 정확히
일치함을 확인. `save_snapshot()` → `list_editable_rows()` → (무수정)
재저장을 3회 반복해도 10행 그대로 유지됨을 확인(수정 전엔 1회 재저장만
으로 10→20행 증식했던 것과 대조). `own_level_name()`으로 조직도를
순회해 2D/NAND/공정/인사그룹 등 모든 노드가 자기 이름을 정확히
표시함을 확인. org_name_wd가 없는 2단계 placeholder 노드("공정"에
아무도 직접 소속 안 된 경우)를 별도로 만들어 reshape가 이 행을
건드리지 않고 원본 라벨을 그대로 보존함(own_level_name도 정상 폴백)을
확인. `scripts/migrate_team_refer_reshape.py`는 코드 변경 없이 새
...
## 2026-09-17 (12): 보유 전문성 조직도 — 연구원 목록이 직접 붙은 노드는
기본 접힘으로 변경

사용자 보고: "하위부서 아래의 연구원들이 디폴트값으로 보이는 중" —
`org_tree_html()`이 team_layer<=2 노드를 무조건 기본으로 펼쳐서
보여줬는데, 그 조직 자체가 리프라 연구원이 직접 배정된 경우(예: 2단계
"2D")는 그 연구원 목록까지 같이 펼쳐져 클릭 없이 바로 보였다. 이 노드
자신에게 붙은 콘텐츠(node_content_fn(node)의 extra)가 있으면
team_layer와 무관하게 접도록 수정 — 순수 하위조직 탐색용(자식만 있고
자신에게는 목록이 없는) 노드만 계속 기본으로 펼쳐진다. `process_researcher_
expertise.py`/`process_researcher_similarity.py` 둘 다 같은 `org_tree_html()`
을 공유해 자동으로 반영됨. 가상 트리(그룹 노드+리프 노드 혼합)로 재현
테스트해 의도한 대로 동작함을 확인.

## 2026-09-17 (13): team_refer 인텔이크 전처리 — 사번/성명/직책 자동 채움

기존엔 인력현황 원본→인텔이크 변환(`pipeline/team_refer_intake.py`)이
사번/성명/직책을 전부 빈 값으로 뒀는데(그 조직의 대표 책임자 판단이
어려워서), 원본에 있는 다른 헤더들로 판단 가능하다는 사용자 요청에
따라 자동 채움 로직을 추가했다.

**직책 판단**(`_compute_title()`, 우선순위 순, 사용자 확정): 1)
"직책명" 값이 있으면 그 값. 2) (1이 없을 때) "글로벌직책명" 값이
있으면 그 값 — 단 "고문"이 포함되면 그 사람은 책임자 후보에서 완전히
제외(3번 체크로도 안 넘어감). 3) (1·2가 전부 없을 때) "직무프로필명"에
"(M)"이 포함되면(부분 일치) "PM". 셋 다 아니면 직책 없음. "직책명"과
"글로벌직책명"은 실데이터에서 한 행에 동시에 값이 있는 경우가 없다고
사용자가 확인.

**매핑**(`_build_leader_lookup()`): 직책이 계산된 행만 대상으로 그
행의 "비공식소속부서명"(org_name_wd)을 키 삼아 "사원번호"/"성명"/
계산된 직책을 모아 조회표를 만들고, `transform()`이 만드는 조직 단위별
1행에 org_name_wd로 매칭해 채운다. 같은 org_name_wd에 후보가 여러 명
있는 경우는 실데이터에 없다고 사용자가 확인(있어도 마지막 값이
채택되는 안전망만 둠). 매칭되는 후보가 없는 조직은 기존과 동일하게
빈 값. 필요한 5개 헤더("직무프로필명"/"직책명"/"글로벌직책명"/
"사원번호"/"성명") 중 하나라도 원본에 없으면 이 채움 단계만 건너뛰고
(전부 빈 값) 나머지 변환은 그대로 진행 — 원본 판정(`is_raw_format()`)
기준에는 포함하지 않는다.

**검증**: 팀장(직책명)/글로벌직책명(고문 아님)/직무프로필명 "(M)"/
글로벌직책명 "고문"(제외, "(M)"도 있었지만 무시됨 확인)/아무 정보
없음 5가지 케이스를 합성 데이터로 만들어 전부 기대한 값으로 채워짐을
확인. 5개 헤더가 전혀 없는 원본도 크래시 없이 전부 빈 값으로 채워짐을
확인(하위 호환). `python3 -m py_compile`/`app` import 정상.

## 2026-09-17 (14): 사번/성명/직책 자동 채움 — 제외 마커 확장 + 과제
변형 간 전파

(13)에 이어 사용자 요청 2건 추가 반영:
1. "직책명" 헤더도 "고문"/"자문"이 포함되면(부분 일치 — "비상근고문"도
   해당) 완전히 제외하도록 확장(기존엔 직책명에는 제외 규칙이 없었음).
2. "글로벌직책명" 헤더에 "회장" 포함 시에도 완전히 제외하도록 추가
   (기존 "고문"에 이어).

추가로 신규 요청 — "(D) 산화수소전자원자"/"(E) 산화수소전자원자"처럼
"(알파벳 한 글자) 과제명" 형식으로 같은 과제명을 공유하는 org_name_wd
변형이 있을 때, 그중 한 변형에서만 책임자를 찾을 수 있어도 나머지
변형에 그대로 전파하는 `_propagate_project_variant_leaders()`를 추가.
`_project_base_name()`(정규식 `^\(([A-Za-z])\)\s*(.+)$`)으로 과제명을
추출해 그룹핑하고, 그룹 안에 책임자를 이미 찾은 변형이 있으면 나머지
(책임자 없는) 변형에 setdefault로 채운다(이미 자기 책임자가 있는
변형은 덮어쓰지 않음).

**검증**: 직책명="비상근고문"(제외, 직무프로필명에 "(M)"도 있었지만
무시됨)/글로벌직책명="명예회장"(제외)/글로벌직책명="기술고문"(기존
규칙 유지 확인)/직책명="경영자문"(제외)/정상 케이스(직책명="팀장")/
"(D)"에만 책임자 있고 "(E)"는 전파받는 케이스 6가지를 합성 데이터로
만들어 전부 기대한 값과 일치함을 확인. `python3 -m py_compile`/`app`
import 정상.

## 2026-09-17 (15): 팀/리더 참조 — 미리보기 삭제 + 과거 인력현황 대량
소급 반영 스크립트 신설

사용자 요청 3건:
1. "엑셀 파일로 한번에 반영"에 인력현황 원본을 그대로 올리면 자동
   변환되는 기능은 이미 2026-09-16에 구현돼 있어(`process()`의
   `team_refer_intake.is_raw_format()` 자동 감지) 추가 작업 없음 —
   확인만 완료.
2. **미리보기 기능 삭제**: `pipeline/process_team_refer.py`의
   `parse_pasted_table()`/`preview_transform()`, `services/team_refer_
   store.py`의 `preview_pasted_table()`, `pages/admin.py`의
   `_team_refer_preview_section()`/`_team_refer_preview_table()`/콜백과
   탭 삽입 지점을 전부 제거.
3. **과거 인력현황 대량 소급 반영**: 웹 UI 대신 CLI 스크립트로 구현하는
   게 더 간단하다고 판단해(임시 화면을 만들었다가 나중에 지우는 것보다
   스크립트 파일 하나 지우는 게 훨씬 가벼움) 제안, 사용자 동의 후 진행.

**신설 `scripts/backfill_team_refer_history.py`**: 과거 여러 달치
인력현황 원본(파일당 1개월, ~100개)을 한 폴더에 모아두고 실행하면:
- 각 파일의 "인원실적년도"/"인원실적월" 헤더 값을 읽어 그 달 **말일**을
  valid_date로 계산(파일명이 아니라 파일 내용 기준 — `calendar.
  monthrange()`로 윤년 등 정확히 계산).
- 필수 헤더(`team_refer_intake._SRC_HEADERS` 3개 + 인원실적년도/월 2개)
  중 하나라도 없는 파일은 처리 전에 걸러내고 사유와 함께 보고(사용자
  확정 — "처음에 필요한 헤더들이 있는지 확인해주는 작업을 삽입").
- 파일명과 무관하게 계산된 날짜 오름차순(오래된 시점부터)으로 자동
  정렬해 처리(사용자 확정). 같은 (연,월) 파일이 여러 개면 정렬 후
  나열 순서상 나중 파일이 최종값(natural key upsert가 자동으로
  덮어씀, 사용자 확정 — "마지막에 처리된 걸 최종값으로").
- 파일마다 `pipeline.process_team_refer.process(raw_dir=<그 파일만
  담은 임시 폴더>, valid_date=<계산된 말일>, skip_tombstone=True)`를
  호출 — 관리자 화면 "엑셀 파일로 한번에 반영"과 완전히 동일한 변환
  로직(인력현황 원본 자동 감지 → team_refer_intake.transform() → 사번/
  성명/직책 자동 채움까지)을 그대로 재사용한다(사용자 확정 — "책임자
  정보 자동 채움으로 해줘").

**중요 발견 및 수정 — `process()`에 `skip_tombstone` 매개변수 추가**:
`tombstone_missing_dep_ids()`는 "현재(전체 파일 기준 가장 최근 날짜)
살아있는 dep_id인데 이번 실행 결과에 없으면 사라진 것"으로 처리하는데,
이미 미래 시점(예: 2026-09, 실제 "현재")이 team_refer.csv에 쌓여 있는
상태에서 그보다 과거 시점(예: 2020-03)을 나중에 소급 반영하면, 미래
에만 존재하는 조직이 전부 "2020-03에 사라짐"으로 잘못 톰스톤 처리되는
문제를 구현 중 발견했다 — "현재" 조직도 표시 자체는 안 깨지지만(dep_id
별 가장 최근 날짜를 고르므로), 기간 지정 조회(특정 시점 기준 조직도
재구성)가 이 잘못된 톰스톤을 "그 시점의 실제 상태"로 오인해 과거
조직도가 깨진다. `process()`에 `skip_tombstone: bool = False`(기본값
유지, 기존 웹 업로드/그리드 저장에는 영향 없음)를 추가하고, 이
백필 스크립트에서만 `skip_tombstone=True`로 호출해 문제를 원천 차단.

**검증**: (a) 파일명과 실제 시점을 일부러 뒤바꾼 4개 파일 + 필수 헤더
누락 1개 파일로 헤더 검증/내용 기준 자동 정렬이 정확함을 확인(파일명
알파벳순이 아니라 계산된 날짜순으로 처리됨), 같은 달 중복 파일 2개 중
나중에 나열된 파일 값이 최종 반영됨을 확인. (b) "현재"(2026-09) 상태에
2020년엔 없던 신규조직을 미리 만들어두고 2020-03 파일을 소급 반영해도
그 신규조직이 2020-03에 잘못 톰스톤되지 않고, "현재" 조직도에도 여전히
살아있음을 확인(skip_tombstone 효과 검증). `python3 -m py_compile`/
`app` import 정상.

## 2026-09-17 (16): team_refer 기간 지정 조회 — 좁은 구간이면 아무것도
안 나오던 문제 수정

과거 인력현황 대량 소급 반영을 실사용해보던 사용자가 "기간 지정 조회
시 가장 근접한 이전 시점(예: 2023년 7월 말)으로 보여주는 줄 알았는데
아닌 것 같다"고 지적 — 실제로 확인해보니 `_latest_rows_in_period()`
(`pipeline/rd_specialist_markdown.py`)가 `[start, end]` 구간 **안에**
스냅샷이 있어야만 그 dep_id를 결과에 포함시켰다. team_refer는 보통
월말에만 스냅샷이 쌓이는데, 좁은 구간(예: 2023-08-10~2023-08-20)을
지정하면 그 구간 안에 월말 스냅샷이 하나도 없어 아무것도 안 나오는
버그였다.

**구분**: `services/period_snapshot.py`의 `resolve_period_snapshot()`
(연구원 이력 조회 전용, researchers_history/evaluations_history 등에
쓰임)은 "구간 안에 있어야 함"이 의도된 설계이고 "시점 하나만 필요하면
아주 이른 시작일로 호출하라"는 사용 패턴이 이미 문서화돼 있어 건드리지
않았다 — `_latest_rows_in_period()`는 team_refer 전용 함수라 범위가
안전하게 격리됨을 확인 후 이 함수만 수정.

**수정**: `start`를 하한으로 강제하지 않고, `end` 이전(포함)의 스냅샷
중 dep_id별 가장 최근 것을 그대로 채택하도록 변경(이 함수의 원래
취지인 "그 기간 기준 dep_id별 최신 스냅샷"에 맞춤) — 그 조직이 실제로
그 시점 훨씬 전에 마지막 갱신됐어도, 그 뒤로 톰스톤(deleted='Y')되지
않았다면 "그 시점까지도 존재했다"고 본다(실제로 사라졌다면 톰스톤이
이후 시점을 지나면서 걸러줌 — (15)의 reconcile_team_refer_tombstones.py
가 이 전제를 보장). `end` 이후 스냅샷은 여전히 제외해 미래 시점 데이터가
과거 조회에 섞여 들어가지 않게 유지. `start` 인자 자체는 호출부 시그니처
호환을 위해 그대로 남겨두되 함수 내부에서는 더 이상 쓰지 않는다.

**검증**: 좁은 구간(2018-08-10~2018-08-20)으로 조회하면 그 이전 월말
스냅샷(2018-07-31)으로 정상 폴백됨을 확인, 이미 그 이전에 톰스톤된
...
## 2026-09-22 (5): Confluence 조회를 atlassian-python-api 대신 requests
직접 호출로 전면 교체

사용자가 IT 지원 페이지 안내대로 만든 curl(게이트웨이 URL + `Authorization:
Bearer`/`X-Dep-Ticket`/`X-Data-Classification`/`accept: application/json`
헤더)은 **200으로 정상 성공**하는데, 완전히 같은 값을 `.env`에 넣고
`python3 pipeline/run_ready.py`를 돌리면 여전히 `HTTPError: Client must be
authenticated`가 남. 헤더 이름 오타(X-Data-Classfication)를 의심했으나
사용자가 확인 — 옮겨적으며 생긴 오타였고 실제로는 정상 철자(X-Data-
Classification)가 맞았음. 즉 URL/헤더 값 자체는 curl과 완전히 동일한데도
라이브러리를 거친 호출만 실패 — `atlassian-python-api`(설치 안 된 이
개발 샌드박스에서는 소스를 직접 확인할 수 없음)가 세션에 미리 심어둔
헤더를 실제 요청 시점에 내부적으로 다시 만지고 있을 가능성이 유력하다고
판단.

**결정**: 라이브러리 내부 동작의 불확실성을 아예 없애기 위해,
`get_page_by_id()`/`get_page_by_title()`(atlassian-python-api)를 더 이상
쓰지 않고 **requests로 REST API를 직접 호출**하도록 전면 교체 —
사용자가 curl로 이미 검증한 요청(URL/헤더 전부)을 코드가 한 글자도
다르지 않게 그대로 재현한다.

**변경**: `pipeline/confluence_client.py`
  - `_get_client(base_url)`(atlassian.Confluence 객체 생성) 삭제,
    `_request_headers()`(Accept+인증+게이트웨이 헤더 조합) +
    `_get_session()`(헤더 미리 심어둔 `requests.Session`, 프로세스당 캐시)로
    교체.
  - `fetch_page_text()`가 `session.get(f'{base_url}/rest/api/content/
    {page_id}', params={'expand': 'body.storage,title'})`를 직접 호출(페이지
    ID로 못 찾으면 `/rest/api/content?spaceKey=...&title=...`로 검색형
    호출, `results[0]`을 그 페이지로 취급) — 응답 JSON 구조
    (`{"title":..,"body":{"storage":{"value":...}}}`)는 REST API 표준
    스펙 그대로라 기존 파싱 로직(`page.get('title')`/`page.get('body')...`)은
    변경 없이 재사용.
  - 200이 아닌 응답은 상태코드+응답 본문 앞부분을 그대로 `ConfluenceError`
    메시지에 담아, 다음에 또 실패하면 원인(예: 게이트웨이의 XML fault
    본문)이 로그에 바로 보이게 함.

**검증**: `requests.Session.get`을 mock으로 대체해 (1) 정상 응답(200) 시
사용자가 curl로 검증한 것과 완전히 동일한 URL(`https://api.example1.
samsungds.net/rest/api/content/1234123456`)·파라미터(`expand=body.storage,
title`)·헤더(`Authorization`/`X-Dep-Ticket`/`X-Data-Classification`/
`Accept`)로 요청이 나가고 본문이 정상 파싱됨을 확인, (2) 401 응답(XML
본문 포함) 시 `ConfluenceError`에 상태코드+본문이 그대로 담겨 나오는 것
확인. `python3 -m py_compile` 통과. atlassian 패키지 의존성 자체는
