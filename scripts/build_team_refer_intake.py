...
백필 단계 신설):
  ...
  사번/성명/직책 → 원본의 "직책명"/"글로벌직책명"/"직무프로필명"/
    ...
    (2026-09-17 추가, pipeline/team_refer_intake.py 모듈 docstring의
    _compute_title()/_build_leader_lookup() 설명 참고 — 직책명 >
    글로벌직책명("고문" 포함 시 완전 제외) > 직무프로필명의 "(M)"→"PM"
    순으로 판단, 같은 조직에 후보가 여러 명 있는 경우는 실데이터에 없다고
    사용자가 확인). 이 5개 헤더 중 하나라도 원본에 없으면 이 채움 단계만
    건너뛰고(전부 빈 값) 나머지 변환은 그대로 진행한다.
