...
(`_fill_upper_level()`, 사용자 확정 2026-09-02):
  ...
  - (원래부터 A가 채워져 있던 행 포함, 모든 행 대상) A가 "대표이사"면 B
    값으로 그대로 교체하고, "종합기술원"/"SAIT"면(2026-09-02 정정 — 처음엔
    셋 다 무조건 B로 교체했으나, 이 둘은 무조건 B로 덮지 않고) 갑의 B를
    을에서 한 번 더 조회해(d와 동일 방식) 더 정밀한 1단계부서명으로
    교체한다(못 찾으면 e와 동일하게 A=B로 폴백, 사용자 확정) — 예를 들어
    을에 "종합기술원"의 상위가 "SAIT"라는 행이 있으면 A가 "종합기술원"에서
    한 단계 더 정밀한 "SAIT"로 바뀔 수 있다.
