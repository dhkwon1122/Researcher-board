...
process_researcher_similarity.py 등이 그대로 읽는 고정 파일명, 매 실행마다
덮어씀)은 전혀 건드리지 않는다. 그와는 별개로, 실행 시각을 파일명에 붙인
사본을 data/processed/result/<subfolder>/ 아래에 추가로 남겨 이력이 누적되도록
한다(덮어쓰기 없음). process_project_expertise.py, process_researcher_expertise.py,
process_project_researcher_fit.py, process_researcher_similarity.py가 공용으로
사용한다.
"""
