...
실패해 빈 문자열로 남은) 저널만 재조회한다(누적 재사용). 단순히 캐시에 키가
있는지가 아니라 값이 실제로 있는지로 판단해야, 실패했던 저널이 캐시에 빈
값으로 영구히 박제되지 않는다.
...
Output:
  data/processed/journal_authority.json  (저널명별 권위도 평가 캐시)
...
CACHE_PATH = os.path.join(OUT_DIR, 'journal_authority.json')
...
def _fetch_authority(journal: str) -> str:
    prompt = (
        f'학술지/저널명: {journal}\n'
...
def update_authority(journals: list, cache: dict, force: bool = False) -> dict:
    """캐시에 실제 평가 값이 채워진 저널은 건너뛰고, 값이 비어 있는 저널(신규거나
    이전 조회가 실패해 빈 문자열로 남은 것)만 사내 LLM으로 재조회한다(누적 재사용).
    force=True(--refresh-journals)면 캐시 값과 무관하게 전달된 저널 전체를 다시
    조회한다. 호출부가 로드/저장을 위임하므로, 이 함수 안에서 저장까지 마친다
    (배치 처리 중간에 실패해도 이미 평가한 저널은 남도록).

    동시 호출 허용치(llm_client.max_concurrency)만큼 스레드풀로 동시 조회한다
    — 전용 vLLM 서버(thinkingcap)는 continuous batching으로 여러 요청을
    한꺼번에 처리하는 편이 순차 호출보다 훨씬 빠르다. 개별 저널 조회 중
    예기치 못한 예외가 나도(run_concurrent가 잡아 반환) 다른 저널 조회는
    ...
    tasks = [(lambda j=j: _fetch_authority(j)) for j in targets]
...
def process(force: bool = False) -> bool:
    ...
    cache = load_cache()
    update_authority(journals, cache, force=force)
    return True


if __name__ == '__main__':
    process(force='--refresh-journals' in sys.argv)
